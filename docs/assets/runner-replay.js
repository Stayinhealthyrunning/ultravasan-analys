...
(function(root,factory){
  ...
  class ReplayController{
    constructor(element,model,media){
      this.element=element;this.model=model;this.media=media;this.distance=0;this.playing=false;this.destroyed=false;this.frameId=null;this.lastFrame=0;this.lastTextUpdate=0;this.lastAnnouncedSegment=-1;this.audioFadeId=null;this.resizeObserver=null;this.elevationLogicalWidth=0;this.motionReduced=typeof matchMedia==='function'&&matchMedia('(prefers-reduced-motion: reduce)').matches;
      this.scrubber=element.querySelector('[data-replay-scrubber]');this.speedSelect=element.querySelector('[data-replay-speed]');this.playButton=element.querySelector('[data-replay-action="play"]');this.audio=element.querySelector('[data-replay-audio]');this.mapSvg=element.querySelector('.runner-replay-map svg');this.mapWorld=element.querySelector('[data-map-world]');this.mapView=fitMapView();this.renderedMapView=fitMapView();this.mapAnimationId=null;this.mapDrag=null;this.mapWasDragged=false;this.pinch=null;this.refreshElevation(false);this.setupAudio();this.bind();this.observeElevation();this.applyMapView(false);this.setDistance(0,true,true);
    }
    ...
    refreshElevation(rebind=true){const chart=this.query('.runner-elevation-chart');if(!chart||!this.model.elevationProfile.length)return;const rect=chart.getBoundingClientRect(),width=Math.max(360,Math.round(ELEV_H*rect.width/(rect.height||ELEV_H)));if(!rect.width||Math.abs(width-this.elevationLogicalWidth)<2)return;chart.outerHTML=renderElevation(this.model,width);this.elevationLogicalWidth=width;if(rebind){this.bindElevation();this.setDistance(this.distance,false,true)}}
    observeElevation(){const section=this.query('.runner-elevation-section');if(!section||typeof ResizeObserver!=='function')return;this.resizeObserver=new ResizeObserver(()=>this.refreshElevation(true));this.resizeObserver.observe(section)}
    ...
    zoomMap(factor,focus={x:SVG_W/2,y:SVG_H/2}){this.mapView=zoomMapView(this.mapView,this.mapView.scale*factor,focus);if(this.mapView.follow&&this.mapView.scale>1){const coordinate=stateAt(this.model,this.distance).coordinate;if(coordinate){const position=this.model._mapProjection.project(coordinate);this.mapView=followMapView(this.mapView,{x:position[0],y:position[1]})}}this.applyMapView(true)}
    fitMap(){this.mapView=fitMapView();this.applyMapView(true)}
    setMapFollow(active,recenter=false){this.mapView={...this.mapView,follow:Boolean(active)};if(active&&recenter&&this.mapView.scale>1){const coordinate=stateAt(this.model,this.distance).coordinate;if(coordinate){const position=this.model._mapProjection.project(coordinate);this.mapView=followMapView(this.mapView,{x:position[0],y:position[1]})}}this.applyMapView(true)}
