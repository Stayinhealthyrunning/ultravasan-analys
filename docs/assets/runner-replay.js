...
(function(root,factory){
  ...
  const ELEV_W=920,ELEV_H=190,ELEV_PAD={l:34,r:12,t:18,b:48};
  ...
  function createModel({race,result,route,raceCheckpoints=[],splits=[]}){
    ...
    for(const cp of checkpoints.slice(1)){
      ...
      if(split&&finite(split.elapsed_seconds)){const isFinish=cp.distance>=totalDistance-.05;anchors.push({distance:cp.distance,time:Number(split.elapsed_seconds),rank:finite(split.place_overall)?Number(split.place_overall):null,classRank:finite(split.place_class)?Number(split.place_class):(isFinish&&finite(result?.class_place)?Number(result.class_place):null),name:cp.name,key:cp.key,kind:isFinish?'finish':'checkpoint',split})}
    }
    const finishCp=checkpoints.at(-1),hasFinishAnchor=anchors.some(a=>a.distance>=totalDistance-.05);
    if(!hasFinishAnchor&&finite(result?.finish_seconds)&&Number(result.finish_seconds)>0)anchors.push({distance:totalDistance,time:Number(result.finish_seconds),rank:finite(result.overall_place)?Number(result.overall_place):null,classRank:finite(result.class_place)?Number(result.class_place):null,name:finishCp?.name||'Mora',key:finishCp?.key||'finish',kind:'finish'});
  ...
  function stateAt(model,distance){
    ...
    return {distance:d,time,coordinate,elevation,grade:terrain?.grade??null,cumulativeAscent:terrain?.cumulativeAscent??null,cumulativeDescent:terrain?.cumulativeDescent??null,ascentRemaining:finite(terrain?.cumulativeAscent)&&finite(model.route?.total_ascent_m)?Math.max(0,Number(model.route.total_ascent_m)-terrain.cumulativeAscent):null,descentRemaining:finite(terrain?.cumulativeDescent)&&finite(model.route?.total_descent_m)?Math.max(0,Number(model.route.total_descent_m)-terrain.cumulativeDescent):null,segment,lastKnownRank:lastAnchor?.rank??null,lastKnownClassRank:lastAnchor?.classRank??null,rankIsExact:atOfficial&&lastAnchor?.rank!=null,classRankIsExact:atOfficial&&lastAnchor?.classRank!=null,previousCheckpoint,nextCheckpoint,remaining:Math.max(0,model.totalDistance-d),finished:d>=model.maxDistance-.001};
  }
  ...
  function renderElevation(model){
    ...
    const cps=model.checkpoints.map((cp,index)=>{const elevation=elevationAtDistance(model.elevationProfile,cp.distance);if(!finite(elevation))return'';const x=p.x(cp.distance),labelY=ELEV_H-7-(index%3)*12,anchor=x<58?'start':x>ELEV_W-58?'end':'middle',full=cleanName(cp.name),short=cleanName(cp.short||cp.name);return `<g class="runner-elevation-checkpoint" data-elevation-checkpoint="${esc(cp.key)}"><line x1="${x.toFixed(1)}" x2="${x.toFixed(1)}" y1="${p.y(elevation).toFixed(1)}" y2="${(labelY-9).toFixed(1)}"/><circle cx="${x.toFixed(1)}" cy="${p.y(elevation).toFixed(1)}" r="3"/><title>${esc(full)} · ${fmtDistance(cp.distance)} · ${Math.round(elevation)} m</title><text class="runner-elevation-label-full" x="${x.toFixed(1)}" y="${labelY.toFixed(1)}" text-anchor="${anchor}">${esc(full)}</text><text class="runner-elevation-label-short" x="${x.toFixed(1)}" y="${labelY.toFixed(1)}" text-anchor="${anchor}">${esc(short)}</text></g>`}).join('');
    ...
    return `<div class="runner-elevation-chart"><svg viewBox="0 0 ${ELEV_W} ${ELEV_H}" role="img" aria-label="Höjdprofil med samtliga kontroller och synkroniserad positionsmarkör"><line class="runner-elevation-baseline" x1="${p.x(0)}" x2="${p.x(model.totalDistance)}" y1="${ELEV_H-ELEV_PAD.b}" y2="${ELEV_H-ELEV_PAD.b}"/><path class="runner-elevation-area" d="${area}"/><path class="runner-elevation-outline" d="${full}"/>${colored}${cps}${highMarker}<text class="runner-elevation-axis" x="4" y="${(p.y(p.max)+4).toFixed(1)}">${Math.round(p.max)} m</text><text class="runner-elevation-axis" x="4" y="${(p.y(p.min)+4).toFixed(1)}">${Math.round(p.min)} m</text><line class="runner-elevation-position" x1="${p.x(0)}" x2="${p.x(0)}" y1="${ELEV_PAD.t}" y2="${ELEV_H-ELEV_PAD.b}"/><circle class="runner-elevation-dot" cx="${p.x(0)}" cy="${p.y(model.elevationProfile[0][1])}" r="6"/><rect class="runner-elevation-hit" x="${ELEV_PAD.l}" y="0" width="${ELEV_W-ELEV_PAD.l-ELEV_PAD.r}" height="${ELEV_H}" tabindex="0" role="slider" aria-label="Dra position längs höjdprofilen" aria-valuemin="0" aria-valuemax="${model.maxDistance.toFixed(1)}" aria-valuenow="0"/></svg></div>`;
  }
  ...
  class ReplayController{
    ...
    reset(){this.pause();if(this.speedSelect)this.speedSelect.value='60s';this.fitMap();this.setDistance(0,true,true);if(this.audio){this.audio.pause();try{this.audio.currentTime=0}catch{}}if(this.playButton)this.playButton.innerHTML='▶ <span>Spela loppet</span>'}
    ...
    destroy(){this.destroyed=true;this.playing=false;if(this.frameId)cancelAnimationFrame(this.frameId);if(this.audioFadeId)cancelAnimationFrame(this.audioFadeId);if(this.mapAnimationId)cancelAnimationFrame(this.mapAnimationId);if(this.audio){this.audio.pause();try{this.audio.currentTime=0}catch{}}}
  }
  ...
  return {PACE_COLORS,NEUTRAL_COLOR,routeForRace,pointAtDistance,terrainAtDistance,elevationAtDistance,paceColor,createModel,stateAt,timeAtDistance,distanceAtTime,buildInsights,mapProjection,fitMapView,zoomMapView,panMapView,followMapView,formatClassPlace,render,mount,stopActive,motionAllowed,fmtTime,fmtPace,cleanName};
});
