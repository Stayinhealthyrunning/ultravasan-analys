...
(function(root,factory){
  ...
  function elevationAtDistance(profile,distance){
    ...
    return Number(a[1])+(Number(b[1])-Number(a[1]))*t;
  }
  ...
  function createModel({race,result,route,raceCheckpoints=[],splits=[]}){
    ...
    const elevationProfile=Array.isArray(route?.elevation_profile)&&route.elevation_profile.length?route.elevation_profile.filter(p=>finite(p?.[0])&&finite(p?.[1])).map(p=>[Number(p[0]),Number(p[1])]):[];
  ...
  function stateAt(model,distance){
    const d=clamp(distance,0,model.maxDistance),segment=segmentAt(model,d),time=timeAtDistance(model,d),coordinate=pointAtDistance(model.route?.points,d),elevation=elevationAtDistance(model.elevationProfile,d);
    ...
    return {distance:d,time,coordinate,elevation,segment,lastKnownRank:lastAnchor?.rank??null,rankIsExact:atOfficial&&lastAnchor?.rank!=null,previousCheckpoint,nextCheckpoint,remaining:Math.max(0,model.totalDistance-d),finished:d>=model.maxDistance-.001};
  }
    ...
    return `<div class="runner-replay" data-runner-replay>
      ...
      <div class="runner-replay-visual">
        ...
        <div class="runner-replay-info" aria-label="Aktuell position och loppinformation">
          ...
          <dl><div><dt>Beräknad loppstid</dt><dd data-replay-value="time">0:00:00</dd></div><div><dt>Höjd</dt><dd data-replay-value="elevation">–</dd></div><div><dt>Delsträcksfart</dt><dd data-replay-value="pace">–</dd></div><div><dt>Placering</dt><dd data-replay-value="rank">Placering saknas</dd></div><div><dt>Återstår</dt><dd data-replay-value="remaining">${fmtDistance(model.totalDistance)}</dd></div></dl>
  ...
  class ReplayController{
    ...
    bind(){
      ...
      const elevationHit=this.query('.runner-elevation-hit');if(elevationHit){const seek=event=>this.seekElevation(event);elevationHit.addEventListener('pointerdown',event=>{elevationHit.setPointerCapture?.(event.pointerId);seek(event)});elevationHit.addEventListener('pointermove',event=>{if(elevationHit.hasPointerCapture?.(event.pointerId))seek(event)});elevationHit.addEventListener('keydown',event=>this.sliderKey(event))}
    }
    ...
    updateText(state){this.setText('distance',fmtDistance(state.distance));this.setText('place',cleanName(state.previousCheckpoint?.name||'På banan'));this.setText('time',fmtTime(state.time));this.setText('elevation',finite(state.elevation)?`${Math.round(state.elevation)} m`:'Höjd saknas');this.setText('pace',fmtPace(state.segment?.pace));this.setText('rank',state.lastKnownRank!=null?`${state.rankIsExact?'Placering':'Senast registrerade placering'} ${state.lastKnownRank}`:'Placering saknas');this.setText('remaining',fmtDistance(state.remaining));this.setText('between',`${cleanName(state.previousCheckpoint?.name||'Start')} → ${cleanName(state.nextCheckpoint?.name||'Sista säkra punkt')}`);this.setText('scrubber-distance',fmtDistance(state.distance));this.setText('scrubber-time',fmtTime(state.time))}
  ...
  return {PACE_COLORS,NEUTRAL_COLOR,routeForRace,pointAtDistance,elevationAtDistance,paceColor,createModel,stateAt,timeAtDistance,distanceAtTime,buildInsights,render,mount,stopActive,motionAllowed,fmtTime,fmtPace,cleanName};
});
