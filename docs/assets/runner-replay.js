...
(function(root,factory){
  ...
  function clampMapView(view){const scale=clamp(view?.scale||1,1,4),minX=SVG_W*(1-scale),minY=SVG_H*(1-scale);return {scale,x:clamp(Number(view?.x||0),minX,0),y:clamp(Number(view?.y||0),minY,0),follow:view?.follow!==false}}
  function zoomMapView(view,nextScale,focus={x:SVG_W/2,y:SVG_H/2}){const current=clampMapView(view),scale=clamp(nextScale,1,4),ratio=scale/current.scale;return clampMapView({...current,scale,x:Number(focus.x)-(Number(focus.x)-current.x)*ratio,y:Number(focus.y)-(Number(focus.y)-current.y)*ratio})}
    ...
    return `<div class="runner-replay" data-runner-replay style="--pace-fast:${PACE_COLORS[0]};--pace-even:${PACE_COLORS[2]};--pace-slow:${PACE_COLORS[4]}">
      <div class="runner-replay-toolbar">
        ...
        <div class="runner-replay-actions">
          ...
          <label>Hastighet<select data-replay-speed aria-label="Välj uppspelningshastighet"><option value="30s">Hela loppet på 30 sekunder</option><option value="60s" selected>Hela loppet på 1 minut</option><option value="120s">Hela loppet på 2 minuter</option><option value="180s">Hela loppet på 3 minuter</option></select></label>
      ...
      <div class="runner-replay-dashboard runner-replay-visual">
        <aside class="runner-replay-now" aria-labelledby="runnerReplayNowTitle">
          ...
          <div class="runner-replay-info" aria-label="Aktuell position och loppinformation">
            ...
            <dl><div><dt>Beräknad loppstid</dt><dd data-replay-value="time">0:00:00</dd></div><div><dt>Delsträcksfart</dt><dd data-replay-value="pace">–</dd></div><div><dt>Totalplacering</dt><dd data-replay-value="rank">Placering saknas</dd></div><div><dt>Klassplacering</dt><dd data-replay-value="class-rank">Inte registrerad ännu</dd></div><div><dt>Höjd</dt><dd data-replay-value="elevation">–</dd></div><div><dt>Lutning</dt><dd data-replay-value="grade">–</dd></div><div><dt>Återstår</dt><dd data-replay-value="remaining">${fmtDistance(model.totalDistance)}</dd></div><div><dt>Stigning kvar</dt><dd data-replay-value="ascent-remaining">–</dd></div></dl>
          </div>
        </aside>
        <section class="runner-replay-map-panel" aria-labelledby="runnerReplayMapTitle">
          <div class="runner-replay-map-heading"><div><p class="eyebrow">GPS-RUTT</p><h4 id="runnerReplayMapTitle">Banan genom landskapet</h4></div><span>${esc(model.route?.source_year||model.race?.year||'')} · ${fmtDistance(model.totalDistance)}</span></div>
          ${renderMap(model)}
          <div class="runner-replay-audio-note" data-replay-audio-note hidden></div>
        </section>
        <aside class="runner-replay-insights" aria-labelledby="runnerReplayInsightsTitle"><div class="runner-replay-side-heading"><p class="eyebrow">ANALYS</p><h4 id="runnerReplayInsightsTitle">Prestationsinsikter</h4></div><div class="runner-replay-insight-grid">${insightCards(model)}</div></aside>
      </div>
  ...
  class ReplayController{
    ...
    fitMap(){this.mapView=fitMapView();this.applyMapView(true)}
    setMapFollow(active,recenter=false){this.mapView={...this.mapView,follow:Boolean(active)};if(active&&recenter&&this.mapView.scale>1){const coordinate=stateAt(this.model,this.progressDistanceKm).coordinate;if(coordinate){const position=this.model._mapProjection.project(coordinate);this.mapView=followMapView(this.mapView,{x:position[0],y:position[1]})}}this.applyMapView(true)}
    ...
    setDistance(distance,announce=false,forceText=false,now=performance.now()){
      ...
      const marker=this.query('.runner-replay-marker');if(marker&&state.coordinate){const pos=this.model._mapProjection.project(state.coordinate);marker.setAttribute('transform',`translate(${pos[0].toFixed(1)} ${pos[1].toFixed(1)})`);if(this.mapView.follow&&this.mapView.scale>1){this.mapView=followMapView(this.mapView,{x:pos[0],y:pos[1]});this.applyMapView(false)}}
      const mapSlider=this.query('.runner-replay-map-hit');mapSlider?.setAttribute('aria-valuenow',this.progressDistanceKm.toFixed(1));this.updateElevationPosition(state);
  ...
  return {PACE_COLORS,NEUTRAL_COLOR,DEFAULT_VOLUME,routeForRace,pointAtDistance,terrainAtDistance,elevationAtDistance,elevationProjection,paceColor,deriveClassPlacements,createModel,wholeRacePace,stateAt,timeAtDistance,distanceAtTime,buildInsights,mapProjection,fitMapView,zoomMapView,panMapView,followMapView,formatClassPlace,render,mount,stopActive,motionAllowed,fmtTime,fmtPace,cleanName};
});
