...
const safeCssToken=v=>String(v??'').toLowerCase().replace(/[^a-z0-9_-]+/g,'-').replace(/^-+|-+$/g,'')||'unknown';
...
async function switchRaceFamily(family,initial=false){
  ...
  try{
    ...
    refreshFilters();applyFilters({historyMode:initial?'replace':'push'});setupMapCompare(true);setupMainRunnerSearch(true);renderRunnerFavorites();
