...
const dataLoader=window.UltravasanDataLoader;
...
async function switchRaceFamily(family,initial=false){if(!['uv90','uv45'].includes(family)||raceSwitchBusy||(!initial&&!raceSwitchHasError&&family===state.raceFamily))return false;raceSwitchBusy=true;raceSwitchHasError=false;setRaceLoading(true);const started=performance.now();try{if(!initial)await waitForRacePaint();state.raceFamily=family;document.body.classList.toggle('race-uv45',family==='uv45');const sw=$('.race-switch');if(sw)sw.dataset.active=family;$('.race-switch-button').forEach(b=>{const active=b.dataset.raceFamily===family;b.classList.toggle('active',active);b.setAttribute('aria-selected',String(active))});const heroReady=updateRaceHero($('#heroHeaderImage'),raceUi[family],initial);document.title=raceUi[family].title;populateRaceYears();await ensureRaceSplits(state.raceId);state.page=1;compareState.selected=[];refreshFilters();applyFilters();setupMapCompare(true);setupMainRunnerSearch(true);try{localStorage.setItem('ultravasan-race-family',family)}catch{}await heroReady;await waitForRacePaint();const remaining=360-(performance.now()-started);if(remaining>0)await waitForRaceDelay(remaining);setRaceLoading(false);if(dataLoader?.status?.().mode==='modular'){dataLoader.ensureFamily(family,window.RaceContracts).then(()=>{refreshDerivedData(state.data);if(state.raceFamily===family)renderAll()}).catch(error=>console.warn('Historikdata kunde inte bakgrundsladdas',error))}return true}catch(error){console.error('Loppväxlingen misslyckades',error);raceSwitchHasError=true;setRaceLoading(false,true);return false}finally{raceSwitchBusy=false}}
async function setupRaceSwitch(){const saved=new URLSearchParams(location.search).get('race')||localStorage.getItem('ultravasan-race-family')||'uv90';$('.race-switch-button').forEach(b=>b.onclick=()=>switchRaceFamily(b.dataset.raceFamily));return switchRaceFamily(saved==='uv45'?'uv45':'uv90',true)}

function refreshDerivedData(d){window.UltravasanDataIndex.ensureSplitsByResult(d);Object.defineProperty(d,'splitEvidence',{value:window.ResultStatus.buildSplitEvidence(d.splits),enumerable:false,configurable:true});const overallPlacements=window.RunnerReplay?.deriveOverallPlacements(d.results,d.splits);if(overallPlacements)Object.defineProperty(d,'overallPlacementLookup',{value:overallPlacements,enumerable:false,configurable:true});const classPlacements=window.RunnerReplay?.deriveClassPlacements(d.results,d.splits);if(classPlacements)Object.defineProperty(d,'classPlacementLookup',{value:classPlacements,enumerable:false,configurable:true});return d}
const hydrateData=d=>{window.RaceContracts.assertKnownRaces(d.races);const rr=new Map(d.results.map(r=>[r.id,r.race_id])),cp=new Map(d.checkpoints.map(c=>[`${c.race_id}|${c.checkpoint_key}`,c]));d.splits.forEach(s=>{const c=cp.get(`${rr.get(s.result_id)}|${s.checkpoint_key}`);if(c){s.checkpoint_name=c.name;s.sequence_no=c.sequence_no;s.distance_km=c.distance_km}if(s.is_estimated==null)s.is_estimated=0});return refreshDerivedData(d)};
async function ensureRaceSplits(raceId){if(!dataLoader?.ensureEdition||raceId==null)return state.data;await dataLoader.ensureEdition(raceId);refreshDerivedData(state.data);return state.data}
...
async function load(){try{const loaded=dataLoader?.loadCore?await dataLoader.loadCore():window.ULTRAVASAN_DATA;if(!loaded)throw new Error('Ingen webbdata kunde laddas.');state.data=hydrateData(loaded);await setup();}catch(e){console.error(e);$('#loading').innerHTML=`<p><strong>Databasen kunde inte läsas.</strong><br>Kontrollera datafilerna i <code>data/</code>.<br><small>${esc(e.message)}</small></p>`;}}
async function setup(){installInfoTooltips();if(state.data.meta.coverage_note){const n=$('#dataNotice');n.hidden=false;n.textContent=state.data.meta.coverage_note}setupSpeedUnitControls();await setupRaceSwitch();const year=$('#yearFilter');year.onchange=async()=>{state.raceId=Number(year.value);setRaceLoading(true);try{await ensureRaceSplits(state.raceId);state.page=1;refreshFilters();applyFilters()}finally{setRaceLoading(false)}};['sexFilter','classFilter','statusFilter'].forEach(id=>$('#'+id).addEventListener('change',()=>{state.page=1;applyFilters()}));$('#resetFilters').onclick=()=>{['sexFilter','classFilter','statusFilter'].forEach(id=>$('#'+id).value='');const search=$('#nameFilter');if(search)search.value='';state.page=1;applyFilters()};$('#prevPage').onclick=()=>{if(state.page>1){state.page--;renderTable()}};$('#nextPage').onclick=()=>{if(state.page<Math.ceil(state.filtered.length/state.pageSize)){state.page++;renderTable()}};$('th[data-sort]').forEach(th=>th.onclick=()=>{const k=th.dataset.sort;state.sortDir=state.sortKey===k?-state.sortDir:1;state.sortKey=k;applyFilters()});const runnerDialog=$('#runnerDialog');$('#runnerDialog .dialog-close').onclick=()=>runnerDialog.close();runnerDialog.addEventListener('close',()=>window.RunnerReplay?.stopActive());setupStatsControls();setupInfoInteractions();$('#generatedAt').textContent=new Date(state.data.meta.generated_at).toLocaleString('sv-SE');$('#databaseSize').textContent=state.data.results.length.toLocaleString('sv-SE');$('#splitCount').textContent=(state.data.meta?.modular_data?Object.values(state.data.meta.modular_data.editions||{}).reduce((sum,item)=>sum+(item.splits||0),0):state.data.splits.length).toLocaleString('sv-SE');$('#loading').classList.add('hidden')}
...
async function openRunner(id){
  const r=state.data.results.find(x=>x.id===id);if(!r)return;await ensureRaceSplits(r.race_id);const race=state.data.races.find(x=>x.id===r.race_id),splits=splitsForResult(id).slice().sort((a,b)=>a.sequence_no-b.sequence_no),route=runnerRouteForRace(race),raceCheckpoints=state.data.checkpoints.filter(x=>x.race_id===r.race_id).sort((a,b)=>a.sequence_no-b.sequence_no),model=window.RunnerReplay?.createModel({race,result:r,route,raceCheckpoints,splits,dataset:state.data,statusApi:window.ResultStatus});
...
async function openMapWithRunners(selected){
  ...
  if(dataLoader?.ensureEditions)await dataLoader.ensureEditions(runners.map(r=>r.race_id));
