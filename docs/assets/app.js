...
const STATUS_CLASS_TOKENS=Object.freeze({FINISHED:'finished',DNF:'dnf',DNS:'dns',DSQ:'dsq','STARTADE INTE':'startade-inte',UNKNOWN:'unknown'});
const statusClassToken=v=>{const token=String(v??'').trim().toUpperCase();return Object.hasOwn(STATUS_CLASS_TOKENS,token)?STATUS_CLASS_TOKENS[token]:'unknown'};
...
function preferredRaceFamily(){
  ...
  if(query==='uv45'||query==='uv90')return query;
  try{
    const saved=localStorage.getItem('ultravasan-race-family');
    if(saved==='uv45'||saved==='uv90')return saved;
  }catch{}
  return 'uv90';
}
async function switchRaceFamily(family,initial=false){
  if(!['uv90','uv45'].includes(family)||raceSwitchBusy||(!initial&&!raceSwitchHasError&&family===state.raceFamily))return false;
  ...
  try{
    ...
    const nextData=await window.UltravasanDataLoader.loadInitialFamily(family);
    ...
    if(state.dataPhase!=='full')startFamilyCompletion(family);else setSplitLoading(false);
    return true;
  }catch(error){
...
function updateAnalysisGuide(){
  const context=$('#analysisGuideContext'),dataStatus=$('#analysisGuideDataStatus'),filterStatus=$('#analysisGuideFilterStatus');
  if(!context||!dataStatus||!filterStatus||!state.data)return;
  const race=state.data.races.find(item=>String(item.id)===String(state.raceId));
  const familyLabel=state.raceFamily==='uv45'?'Ultravasan 45':'Ultravasan 90';
  context.textContent=race?familyLabel+' · '+race.year:familyLabel;
  const phaseCopy={
    active:'Visar valt lopp. Historik och mellantider laddas i nästa steg.',
    core:'Historiska resultat är laddade. Mellantider och fördjupade analyser kompletteras.',
    full:'Full historik och tillgängliga mellantider är laddade.'
  };
  dataStatus.textContent=phaseCopy[state.dataPhase]||'Datastatus kunde inte fastställas.';
  const filters=[
    ['Kön',$('#sexFilter')?.selectedOptions?.[0]?.textContent,$('#sexFilter')?.value],
    ['Klass',$('#classFilter')?.value,$('#classFilter')?.value],
    ['Klubb/ort',$('#clubFilterSearch')?.value,$('#clubFilter')?.value],
    ['Status',$('#statusFilter')?.value,$('#statusFilter')?.value]
  ].filter(([,label,value])=>value&&String(label||'').trim());
  const shown=state.filtered?.length??0,total=raceResults().length;
  filterStatus.textContent=(filters.length?filters.map(([name,label])=>name+': '+String(label).trim()).join(' · '):'Inga fältfilter är aktiva.')+' Visar '+shown.toLocaleString('sv-SE')+' av '+total.toLocaleString('sv-SE')+' resultat.';
}
...
const resultStatus=r=>window.ResultStatus.classify(r,{hasSplit:state.data?.splitEvidence?.has(r?.id)});
const isStarterResult=r=>resultStatus(r).started;
const isFinishedResult=r=>resultStatus(r).finished;
const isDnfResult=r=>resultStatus(r).dnf;
const isDnsResult=r=>resultStatus(r).dns;
...
function renderPaceChart(){
  const splits=splitsForResults(state.filtered).filter(s=>Number(s.pace_seconds_per_km)>0);
...
function renderTable(){const pages=Math.max(1,Math.ceil(state.filtered.length/state.pageSize));state.page=Math.min(state.page,pages);const start=(state.page-1)*state.pageSize,rows=state.filtered.slice(start,start+state.pageSize);$('#resultsBody').innerHTML=rows.length?rows.map(r=>`<tr data-id="${r.id}" tabindex="0" role="button" aria-label="Öppna loppanalys för ${esc(r.name_as_published)}"><td>${r.overall_place??'–'}</td><td><div class="runner-name">${esc(r.name_as_published)}</div><div class="runner-meta">${r.bib?'#'+esc(r.bib):''}${r.city?' · '+esc(r.city):''}</div></td><td>${esc(r.sex||'–')}</td><td>${esc(r.age_class||'–')}</td><td>${esc(r.club||r.city||'–')}</td><td>${esc(r.nationality||'–')}</td><td class="time">${fmtTime(r.finish_seconds)}</td><td class="time">${fmtPace(r.pace_seconds_per_km)}</td><td><span class="status ${statusClassToken(r.status)}">${esc(r.status)}</span></td></tr>`).join(''):`<tr><td colspan="9" class="empty">Inga resultat matchar filtren</td></tr>`;$$('#resultsBody tr[data-id]').forEach(tr=>{const open=()=>openRunner(Number(tr.dataset.id));tr.onclick=open;tr.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open()}}});$('#pageLabel').textContent=`Sida ${state.page} av ${pages}`;$('#prevPage').disabled=state.page<=1;$('#nextPage').disabled=state.page>=pages;$('#resultCountLabel').textContent=`${state.filtered.length.toLocaleString('sv-SE')} resultat`}
...
function renderRunnerJourney(profile){
  ...
  const cards=rows.map(row=>{
    ...
    const detail=row.source==='start'
      ?window.RunnerAnalysis.journeyStartDescription(profile)
...
function runnerDevelopmentGap(value){
  return Number.isFinite(Number(value))?window.RunnerReplay.formatGap(Number(value)):'–';
}
function runnerDevelopmentPace(row){
  if(!Number.isFinite(Number(row.segment_pace_seconds_per_km))||!Number.isFinite(Number(row.pace_delta_seconds_per_km)))return'–';
  const delta=Math.round(Number(row.pace_delta_seconds_per_km)),comparison=Math.abs(delta)<=1?'i nivå med egen helfart':`${Math.abs(delta)} s/km ${delta<0?'snabbare':'långsammare'}`,index=Number.isFinite(Number(row.pace_index))?` · index ${Number(row.pace_index).toLocaleString('sv-SE',{maximumFractionDigits:0})}`:'';
  return `${fmtPace(row.segment_pace_seconds_per_km)} · ${comparison}${index}`;
}
function renderRunnerDevelopment(model){
  const rows=window.RunnerReplay?.developmentForModel?.(model)||[];
  if(!rows.length)return'';
  const body=rows.map(row=>`<tr class="runner-development-row" data-development-distance="${Number(row.distance_km)}" tabindex="0" role="button" aria-label="Visa ${esc(cleanCheckpointName(row.checkpoint_name))} i Replay"><th>${esc(cleanCheckpointName(row.checkpoint_name))}<small>${Number(row.distance_km).toLocaleString('sv-SE',{minimumFractionDigits:1,maximumFractionDigits:1})} km</small></th><td class="time">${fmtTime(row.elapsed_seconds)}</td><td>${esc(runnerDevelopmentGap(row.field_gap_seconds))}</td><td>${esc(runnerDevelopmentGap(row.sex_gap_seconds))}</td><td>${esc(runnerDevelopmentGap(row.class_gap_seconds))}</td><td>${row.place_overall??'–'}</td><td>${row.place_class??'–'}</td><td>${esc(runnerDevelopmentPace(row))}</td></tr>`).join('');
  return `<section class="runner-development panel" aria-labelledby="runnerDevelopmentTitle"><div class="runner-section-head"><div><p class="eyebrow">LOPPETS UTVECKLING</p><h3 id="runnerDevelopmentTitle">Checkpoint för checkpoint</h3></div><span class="pill">${rows.length} säkra passager</span></div><p class="runner-development-lead">Gapen jämför löparens faktiska passagetid med medianprofilen för samma loppår och kompletta FINISHED-kohort. Klicka på en rad för att flytta Replay till kontrollen.</p><div class="table-wrap"><table class="runner-development-table"><thead><tr><th>Kontroll</th><th>Tid</th><th>Hela fältet</th><th>Mitt kön</th><th>Min klass</th><th>Totalplats</th><th>Klassplats</th><th>Segment mot egen helfart</th></tr></thead><tbody>${body}</tbody></table></div></section>`;
}
function bindRunnerDevelopment(controller){
  if(!controller)return;
  document.querySelectorAll('#runnerDetail [data-development-distance]').forEach(row=>{
    const seek=()=>controller.setDistance(Number(row.dataset.developmentDistance),true,true);
    row.addEventListener('click',seek);
    row.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();seek()}});
  });
}
...
const placementZoomState={signature:'',domain:null};
const normalizeAppPlacementZoomRect=(start,current,bounds)=>{const clampX=value=>Math.max(bounds.left,Math.min(bounds.right,Number(value))),clampY=value=>Math.max(bounds.top,Math.min(bounds.bottom,Number(value))),x1=clampX(start.x),x2=clampX(current.x),y1=clampY(start.y),y2=clampY(current.y),left=Math.min(x1,x2),top=Math.min(y1,y2),right=Math.max(x1,x2),bottom=Math.max(y1,y2);return{left,top,right,bottom,width:right-left,height:bottom-top}};
let cancelPlacementZoomDrag=()=>{};
function stopPlacementZoomDrag(){const cancel=cancelPlacementZoomDrag;cancelPlacementZoomDrag=()=>{};cancel()}
function resetPlacementZoom(){stopPlacementZoomDrag();placementZoomState.domain=null;renderPlacementScatter()}
function setupStatsControls(){const targetSelect=$('#targetTimeSelect');if(targetSelect)targetSelect.addEventListener('change',event=>{targetSimulatorSelections.set(targetSimulatorKey(),Number(event.target.value));renderTargetSimulator()});['placementSexM','placementSexF'].forEach(id=>$('#'+id)?.addEventListener('change',renderPlacementScatter));$('#placementZoomReset')?.addEventListener('click',resetPlacementZoom)}
function activeSplits(){return splitsForResults(state.filtered)}
...
function renderPlacementScatter(){
  stopPlacementZoomDrag();const el=$('#placementScatter'),reset=$('#placementZoomReset');if(!el)return;
  const showM=$('#placementSexM')?.checked!==false,showF=$('#placementSexF')?.checked!==false;
  if(!showM&&!showF){el.innerHTML='<div class="empty">Välj minst män eller kvinnor</div>';if(reset)reset.disabled=true;return}
  ...
  if(rows.length<2){el.innerHTML='<div class="empty">Minst två placerade löpare behövs för valt kön</div>';if(reset)reset.disabled=true;return}
  const W=760,H=245,p={l:52,r:18,t:14,b:40},edgePadding=8,plot={left:p.l,top:p.t,right:W-p.r,bottom:H-p.b},dataBounds={left:plot.left+edgePadding,top:plot.top+edgePadding,right:plot.right-edgePadding,bottom:plot.bottom-edgePadding},full={minT:Math.min(...rows.map(r=>r.finish_seconds)),maxT:Math.max(...rows.map(r=>r.finish_seconds)),minP:1,maxP:Math.max(...rows.map(r=>r.overall_place))};
  const signature=[state.raceId,showM,showF,rows.length,full.minT,full.maxT,full.maxP].join(':');if(placementZoomState.signature!==signature){placementZoomState.signature=signature;placementZoomState.domain=null}
  ...
  for(let i=0;i<=4;i++){const t=domain.minT+(domain.maxT-domain.minT)*i/4,xx=x(t);out+=svg('text',{x:xx,y:H-12,'text-anchor':'middle'},fmtTime(t).slice(0,-3))}
  rows.filter(r=>r.finish_seconds>=domain.minT&&r.finish_seconds<=domain.maxT&&r.overall_place>=domain.minP&&r.overall_place<=domain.maxP).forEach(r=>{const sex=sexOf(r),color=sex==='M'?'#2563eb':'#db2777',cx=x(r.finish_seconds),cy=y(r.overall_place),title=`${r.name_as_published} · ${sex==='M'?'Man':'Kvinna'} · ${fmtTime(r.finish_seconds)} · plats ${r.overall_place}`,common=`class="placement-scatter-point" tabindex="0" role="img" aria-label="${esc(title)}"`;marks+=sex==='F'?`<path ${common} d="M${cx} ${cy-5} L${cx+5} ${cy} L${cx} ${cy+5} L${cx-5} ${cy} Z" fill="${color}" opacity=".75"><title>${esc(title)}</title></path>`:`<circle ${common} cx="${cx}" cy="${cy}" r="4.3" fill="${color}" opacity=".72"><title>${esc(title)}</title></circle>`});
  ...
  el.innerHTML=`<div class="chart-inline-legend">${legend}${placementZoomState.domain?'<strong>Inzoomad vy</strong>':''}</div><svg viewBox="0 0 ${W} ${H}" aria-label="Tid mot placering. Dra över ett område för att zooma."><defs><clipPath id="placementScatterClip"><rect x="${plot.left}" y="${plot.top}" width="${plot.right-plot.left}" height="${plot.bottom-plot.top}"/></clipPath></defs>${out}<g clip-path="url(#placementScatterClip)"><rect class="placement-zoom-hit" x="${dataBounds.left}" y="${dataBounds.top}" width="${dataBounds.right-dataBounds.left}" height="${dataBounds.bottom-dataBounds.top}"/>${marks}<rect class="placement-zoom-selection" hidden/></g></svg>`;
  ...
  const finish=event=>{if(!drag)return;const rect=drawSelection(point(event)),chartRect=chart.getBoundingClientRect(),minX=6*W/(chartRect.width||W),minY=6*H/(chartRect.height||H);cancel();if(rect.width<minX||rect.height<minY)return;const timeAt=px=>domain.minT+(px-dataBounds.left)*(domain.maxT-domain.minT)/(dataBounds.right-dataBounds.left),placeAt=py=>domain.minP+(py-dataBounds.top)*(domain.maxP-domain.minP)/(dataBounds.bottom-dataBounds.top);placementZoomState.domain={minT:timeAt(rect.left),maxT:timeAt(rect.right),minP:placeAt(rect.top),maxP:placeAt(rect.bottom)};renderPlacementScatter()};
...
function renderDnfFunnel(){
  ...
  const dns=rows.filter(isDnsResult),starters=rows.filter(isStarterResult),dnf=starters.filter(isDnfResult),dsq=rows.filter(isDsqResult);
  if(!dnf.length){el.innerHTML=`<div class="dnf-summary"><strong>Inga registrerade DNF</strong><span>${starters.length.toLocaleString('sv-SE')} startande · ${dns.length.toLocaleString('sv-SE')} DNS · ${dsq.length.toLocaleString('sv-SE')} DSQ.</span></div>`;return}
  const cps=state.data.checkpoints.filter(c=>c.race_id===state.raceId).sort((a,b)=>a.sequence_no-b.sequence_no);
  const byResult=new Map();
  splitsForResults(dnf).filter(s=>Number.isFinite(Number(s.sequence_no))).forEach(s=>{
  ...
  el.innerHTML=`<div class="dnf-summary"><strong>${dnf.length.toLocaleString('sv-SE')} DNF av ${starters.length.toLocaleString('sv-SE')} startande</strong><span>${dns.length.toLocaleString('sv-SE')} DNS/ej start räknas inte som startande · ${dsq.length.toLocaleString('sv-SE')} DSQ redovisas separat.</span></div><div class="dnf-segments">${data.map(x=>`<div class="dnf-segment-row"><span>${esc(x.label)}</span><div><i style="width:${Math.max(4,x.count/max*100)}%"></i></div><strong>${x.count}</strong><small>${Math.round(x.count/dnf.length*100)} % av DNF</small></div>`).join('')}</div>`;
}
...
function renderTargetSimulator(){
  const select=$('#targetTimeSelect'),label=$('#targetTimeLabel'),out=$('#targetTimeResult'),medal=$('#targetMedalTimes');
  if(!select||!label||!out)return;
  if(medal)medal.hidden=state.raceFamily!=='uv90';
  const rows=state.filtered.filter(r=>isFinishedResult(r)&&r.overall_place).sort((a,b)=>a.finish_seconds-b.finish_seconds);
  ...
  const roundedMin=Math.floor(min/60)*60,roundedMax=Math.ceil(max/60)*60;
  const mean=rows.reduce((sum,row)=>sum+Number(row.finish_seconds),0)/rows.length,defaultTime=Math.round(mean/60)*60,key=targetSimulatorKey(),prev=targetSimulatorSelections.get(key)??defaultTime;
  const options=[];
  for(let t=roundedMin;t<=roundedMax;t+=60)options.push(t);
  if(!options.length)options.push(Math.round(min/60)*60);
...
function createMapPayload(selected){
  const raceIds=new Set(selected.map(r=>r.race_id));
  return {
...
function renderHeadToHead(model){
  ...
  const finish=model.whole_course_comparable
    ?`<section class="h2h-finish"><div class="runner-section-head"><div><p class="eyebrow">MÅLGÅNG</p><h3>Sluttid och gap</h3></div><span class="pill">verifierad helbaneserie</span></div><div class="h2h-finish-grid">${model.finish_ranking.map(row=>{
    ...
    :`<aside class="h2h-warning"><strong>Sluttider jämförs inte direkt.</strong><span>De valda resultaten saknar en gemensam verifierad helbaneserie. Tiderna visas därför inte som vinnare/gap, även om checkpointkontraktet kan vara detsamma.</span></aside>`;
...
const INFO_HELP_EXTENDED=[
  ...
  ['#classIndexTable','Klassens starkaste prestationer följer de klasser du har valt i Klassduellen. Rangordningen görs inom samma loppår, kön och åldersklass och kan växlas mellan dominans mot klassmedianen, segermarginal och klasspercentil. Minst fem fullföljande krävs för huvudlistan; mindre underlag märks som reservläge.'],
  ['#raceIndexTable','Loppets starkaste prestationer använder samma mått som klasslistan men rangordnar alla åldersklasser i det aktuella loppåret. Den påverkas inte av klassvalet i Klassduellen.'],
  ['#classHistoryChart','Växla mellan Mediantid och Deltagande. Mediantid visar bara median sluttid för fullföljande och gör tidsutvecklingen lättare att läsa. Medianlinjen binds samman mellan efterföljande tävlingsår med data och bryts vid kalenderluckor utan lopp. Deltagande visar antal startande som staplar; den mörka delen av varje stapel är DNF och tooltip visar både antal och andel. DNS räknas inte som startande. Bansträckningen har förändrats över historiken, så nivåskillnader mellan år är beskrivande och ska inte tolkas som ren prestationsutveckling.'],
  ['#classEvolutionChart','Följ klassernas historiska utveckling år för år. Höjden visar medianfart i min/km bland giltiga fullföljande, där snabbare klasser ligger högre. Bubbelarean visar antal faktiska startande och valda klasser lämnar spår bakåt i historiken. Hovra, klicka eller fokusera en aktuell eller historisk bubbla för exakta värden.'],
  ...
  ['#clubRankings','Byt mått för att rangordna klubbar efter exempelvis antal startande, snabbast median, bredd, fullföljandegrad eller stark avslutning. Måttet Mest förbättrad beräknas endast mellan år inom en uttryckligt verifierad helbaneserie; om minst två användbara jämförbara år saknas visas inget förbättringstal.'],
  ['#clubDna','Klubb/ort-DNA visar fem relativa mått. Fart är ett index där 100 är medianfarten för hela det aktuella fältet; över 100 betyder snabbare klubbmedian och under 100 långsammare. Bredd, uthållighet, avslutning och deltagande visas på en 0–100-skala.'],
  ...
  ['#clubHistoryChart','Jämför valda klubbars och orters starter, målgångar och mediantid över åren. Medianlinjen binds samman mellan efterföljande tävlingsår med data och bryts vid kalenderluckor utan lopp. Eftersom bansträckningen kan ha förändrats är tidslinjen beskrivande, inte ett bevis på ren prestationsförändring. Staplarna använder vänster personaxel och medianlinjen höger tidsaxel.'],
  ...
  ['#percentileLadder','Visar när 10, 25, 50, 75 och 90 procent av kohorten FINISHED i aktuellt huvudfilterurval hade gått i mål. Nivåerna motsvarar Q10, Q25, Q50, Q75 och Q90 och beräknas med samma linjära kvantilmetod som övrig statistik; Q50 är medianen. DNS och DNF ingår inte eftersom de saknar giltig sluttid. Män/Kvinnor-knapparna påverkar endast de sekundära könsvärdena och ändrar inte huvudvärdet Aktuellt urval.'],
  ['#runnerHistory','Sök en löpare och jämför personens genomförda lopp, tider och utveckling över flera år.'],
  ['#fieldFlow','Visar faktiska startande med en sammanhängande följd av exakta, ej estimerade kontrollpassager från första kontrollen. En saknad eller estimerad passage bryter progressionen; en senare råobservation fyller inte luckan. DNF lämnar flödet vid sin sista säkra passage och DNS räknas inte som startande även om råa kontrollvärden finns. Diagrammet visar konservativ källtäckning, inte position mellan kontroller.'],
...
let infoInteractionsReady=false,infoTipsUpgraded=false,infoTipCounter=0;
...
function addCardInfo(card,text){
  ...
  tip.type='button';tip.className='info-tip';tip.setAttribute('aria-label','Visa förklaring');tip.setAttribute('aria-expanded','false');tip.setAttribute('aria-controls',popupId);tip.setAttribute('aria-describedby',popupId);
  tip.innerHTML=`<svg class="info-glyph" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="10" fill="#fff" stroke="#1b7659" stroke-width="1.8"/><circle cx="12" cy="7.2" r="1.25" fill="#1b7659"/><path d="M10.8 10.3h1.8v6.1h1.3" fill="none" stroke="#1b7659" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/></svg><span class="info-popup" id="${popupId}" role="tooltip">${esc(text)}</span>`;
