...
function syncHiddenSet(d,key,next){if(d[key] instanceof Set){d[key].clear();for(const value of next)d[key].add(value);return d[key]}Object.defineProperty(d,key,{value:next,enumerable:false});return next}
function syncHiddenMap(d,key,next){if(d[key] instanceof Map){d[key].clear();next.forEach((value,k)=>d[key].set(k,value));return d[key]}Object.defineProperty(d,key,{value:next,enumerable:false});return next}
const hydrateData=d=>{d=d||{};d.races=Array.isArray(d.races)?d.races:[];d.results=Array.isArray(d.results)?d.results:[];d.checkpoints=Array.isArray(d.checkpoints)?d.checkpoints:[];d.splits=Array.isArray(d.splits)?d.splits:[];window.RaceContracts.assertKnownRaces(d.races);const rr=new Map(d.results.map(r=>[Number(r.id),Number(r.race_id)])),cp=new Map(d.checkpoints.map(c=>[`${Number(c.race_id)}|${c.checkpoint_key}`,c]));d.results.forEach(r=>{r.id=Number(r.id);r.race_id=Number(r.race_id)});d.splits.forEach(s=>{s.result_id=Number(s.result_id);const c=cp.get(`${rr.get(s.result_id)}|${s.checkpoint_key}`);if(c){s.checkpoint_name=c.name;s.sequence_no=Number(c.sequence_no);s.distance_km=c.distance_km==null?null:Number(c.distance_km)}if(s.is_estimated==null)s.is_estimated=0});window.UltravasanDataIndex.refreshSplitsByResult(d);syncHiddenSet(d,'splitEvidence',window.ResultStatus.buildSplitEvidence(d.splits));const overallPlacements=window.RunnerReplay?.deriveOverallPlacements(d.results,d.splits);if(overallPlacements)syncHiddenMap(d,'overallPlacementLookup',overallPlacements);const classPlacements=window.RunnerReplay?.deriveClassPlacements(d.results,d.splits);if(classPlacements)syncHiddenMap(d,'classPlacementLookup',classPlacements);window.ULTRAVASAN_DATA=d;return d};
async function ensureRaceLoaded(raceId){if(!raceId)return state.data;if(window.UltravasanDataLoader){await window.UltravasanDataLoader.ensureRace(raceId);hydrateData(state.data)}return state.data}
async function ensureHistoryLoaded({rerender=true}={}){if(!window.UltravasanDataLoader)return state.data;const before=state.data?.results?.length||0;await window.UltravasanDataLoader.ensureHistory();hydrateData(state.data);if(rerender&&(state.data.results.length!==before)){window.dispatchEvent(new CustomEvent('ultravasan:history-ready'));refreshFilters();applyFilters()}return state.data}
async function ensureFamilyDataLoaded(family=state.raceFamily){if(!window.UltravasanDataLoader)return state.data;await window.UltravasanDataLoader.ensureFamily(family);hydrateData(state.data);window.dispatchEvent(new CustomEvent('ultravasan:family-data-ready',{detail:{family}}));refreshFilters();applyFilters();return state.data}
window.ensureUltravasanRaceData=ensureRaceLoaded;window.ensureUltravasanHistory=ensureHistoryLoaded;window.ensureUltravasanFamilyData=ensureFamilyDataLoaded;
function scheduleHistoryLoad(){const run=()=>ensureHistoryLoaded().catch(error=>console.warn('Historikindex kunde inte laddas',error));if(typeof requestIdleCallback==='function')requestIdleCallback(run,{timeout:1600});else requestAnimationFrame(()=>requestAnimationFrame(run))}
...
async function load(){try{if(window.UltravasanDataLoader){state.data=hydrateData(await window.UltravasanDataLoader.loadInitial());setup();scheduleHistoryLoad();return}if(window.ULTRAVASAN_DATA){state.data=hydrateData(window.ULTRAVASAN_DATA);setup();return}const r=await fetch('data/ultravasan.json',{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);state.data=hydrateData(await r.json());setup();}catch(e){console.error(e);$('#loading').innerHTML=`<p><strong>Databasen kunde inte läsas.</strong><br>Kontrollera att U3-data eller fallbackfilen <code>data/ultravasan-data.js</code> finns bredvid webbplatsen.<br><small>${esc(e.stack||e.message)}</small></p>`;}}
function setup(){installInfoTooltips();if(state.data.meta.coverage_note){const n=$('#dataNotice');n.hidden=false;n.textContent=state.data.meta.coverage_note}setupSpeedUnitControls();setupRaceSwitch();const year=$('#yearFilter');year.onchange=async()=>{state.raceId=Number(year.value);setRaceLoading(true);try{await ensureRaceLoaded(state.raceId);state.page=1;refreshFilters();applyFilters()}finally{setRaceLoading(false)}};['sexFilter','classFilter','statusFilter'].forEach(id=>$('#'+id).addEventListener('change',()=>{state.page=1;applyFilters()}));$('#resetFilters').onclick=()=>{['sexFilter','classFilter','statusFilter'].forEach(id=>$('#'+id).value='');const search=$('#nameFilter');if(search)search.value='';state.page=1;applyFilters()};$('#prevPage').onclick=()=>{if(state.page>1){state.page--;renderTable()}};$('#nextPage').onclick=()=>{if(state.page<Math.ceil(state.filtered.length/state.pageSize)){state.page++;renderTable()}};$$('th[data-sort]').forEach(th=>th.onclick=()=>{const k=th.dataset.sort;state.sortDir=state.sortKey===k?-state.sortDir:1;state.sortKey=k;applyFilters()});const runnerDialog=$('#runnerDialog');$('#runnerDialog .dialog-close').onclick=()=>runnerDialog.close();runnerDialog.addEventListener('close',()=>window.RunnerReplay?.stopActive());setupStatsControls();setupInfoInteractions();$('#generatedAt').textContent=new Date(state.data.meta.generated_at).toLocaleString('sv-SE');$('#databaseSize').textContent=(Number(state.data.meta.total_results)||state.data.results.length).toLocaleString('sv-SE');$('#splitCount').textContent=(Number(state.data.meta.total_splits)||state.data.splits.length).toLocaleString('sv-SE');$('#loading').classList.add('hidden')}
...
async function openRunner(id){
  let r=state.data.results.find(x=>x.id===id);if(!r)return;await ensureRaceLoaded(r.race_id);r=state.data.results.find(x=>x.id===id)||r;const race=state.data.races.find(x=>x.id===r.race_id),splits=splitsForResult(id).slice().sort((a,b)=>a.sequence_no-b.sequence_no),route=runnerRouteForRace(race),raceCheckpoints=state.data.checkpoints.filter(x=>x.race_id===r.race_id).sort((a,b)=>a.sequence_no-b.sequence_no),model=window.RunnerReplay?.createModel({race,result:r,route,raceCheckpoints,splits,dataset:state.data,statusApi:window.ResultStatus});
...
async function openMapWithRunners(selected){
  ...
  for(const raceId of [...new Set(runners.map(r=>r.race_id))])await ensureRaceLoaded(raceId);
  const hydrated=runners.map(r=>state.data.results.find(x=>x.id===r.id)||r);
  const ids=hydrated.map(r=>r.id).join(','),payload=createMapPayload(hydrated),token=`${Date.now()}-${Math.random().toString(36).slice(2,10)}`;
  try{
...
function setupMapCompare(rebuild=false){
  ...
  year.onchange=async()=>{compareState.raceId=year.value==='all'?'all':Number(year.value);if(compareState.raceId==='all')await ensureHistoryLoaded({rerender:false});else await ensureRaceLoaded(compareState.raceId);$('#compareRunnerSearch').value='';hideCompareSuggestions();renderCompareSelection()};
  const search=$('#compareRunnerSearch');const lazyCompareSuggestions=async()=>{try{if(compareState.raceId==='all')await ensureHistoryLoaded({rerender:false});else await ensureRaceLoaded(compareState.raceId);renderCompareSuggestions()}catch(error){console.warn('Kartduellsökning kunde inte ladda vald data',error)}};search.addEventListener('input',lazyCompareSuggestions);search.addEventListener('focus',lazyCompareSuggestions);
...
function setupMainRunnerSearch(rebuild=false){
  ...
  const ensureSearchData=async()=>{if(year.value==='all')await ensureHistoryLoaded({rerender:false});else await ensureRaceLoaded(Number(year.value))};const lazyShow=async()=>{try{await ensureSearchData();show()}catch(error){console.warn('Löparsökning kunde inte ladda vald historik',error)}};input.addEventListener('input',lazyShow);input.addEventListener('focus',lazyShow);year.addEventListener('change',async()=>{input.value='';box.hidden=true;await ensureSearchData();input.focus()});
