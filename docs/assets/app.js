...
const fixedFinishTimeBins=(times,step=1800)=>{const values=(times||[]).map(Number).filter(Number.isFinite).sort((a,b)=>a-b);if(!values.length)return{start:null,step,bins:[]};const start=values[0],count=Math.floor((values.at(-1)-start)/step)+1,bins=Array.from({length:count},(_,i)=>({from:start+i*step,to:start+(i+1)*step,count:0}));for(const value of values)bins[Math.min(bins.length-1,Math.floor((value-start)/step))].count++;return{start,step,bins}};
...
function renderHistogram(times){const el=$('#histogram');if(!times.length){el.innerHTML='<div class="empty">Inga sluttider i urvalet</div>';return}const grouped=fixedFinishTimeBins(times),bins=grouped.bins,counts=bins.map(bin=>bin.count),W=Math.max(650,bins.length*34+58),H=270,p={l:44,r:14,t:12,b:42},cw=(W-p.l-p.r)/bins.length,ymax=Math.max(...counts,1);let s='';for(let i=0;i<=4;i++){const y=p.t+(H-p.t-p.b)*i/4;s+=svg('line',{x1:p.l,y1:y,x2:W-p.r,y2:y,class:'gridline'})+svg('text',{x:5,y:y+4},String(Math.round(ymax*(1-i/4))))}counts.forEach((c,i)=>{const h=(H-p.t-p.b)*c/ymax,x=p.l+i*cw+2,y=H-p.b-h,bin=bins[i],tip=`${c} löpare, ${fmtTime(bin.from)}–${fmtTime(bin.to)}`;s+=`<rect class="bar" x="${x}" y="${y}" width="${Math.max(2,cw-4)}" height="${h}" tabindex="0" role="img" aria-label="${tip}"><title>${tip}</title></rect>`;if(i%Math.max(1,Math.ceil(bins.length/7))===0||i===bins.length-1)s+=svg('text',{x:x,y:H-18},fmtTime(bin.from).slice(0,-3))});s+=svg('line',{x1:p.l,y1:H-p.b,x2:W-p.r,y2:H-p.b,class:'axis'});el.innerHTML=`<svg viewBox="0 0 ${W} ${H}" style="min-width:${W}px">${s}</svg>`;$('#distributionLabel').textContent=`30-minutersintervall från ${fmtTime(grouped.start)} · hälften: ${fmtTime(quantile(times,.25))}–${fmtTime(quantile(times,.75))}`}
