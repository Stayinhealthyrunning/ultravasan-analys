...
function runnerMapMarkup(route,splits){
  if(!route||!Array.isArray(route.points)||route.points.length<2)return '<div class="runner-map-empty">Banrutten saknas för detta loppår.</div>';
  const pts=route.points,lat=pts.map(p=>Number(p[0])),lon=pts.map(p=>Number(p[1])),minLat=Math.min(...lat),maxLat=Math.max(...lat),minLon=Math.min(...lon),maxLon=Math.max(...lon),W=920,H=440,pad=42;
  const x=v=>pad+(Number(v)-minLon)*(W-pad*2)/(maxLon-minLon||1),y=v=>H-pad-(Number(v)-minLat)*(H-pad*2)/(maxLat-minLat||1),cps=route.checkpoints||[];
  let paths='';for(let i=1;i<cps.length;i++){const seg=runnerRouteSegment(route,cps[i-1].distance_km,cps[i].distance_km);if(seg.length<2)continue;const d=seg.map((q,j)=>`${j?'L':'M'}${x(q[1]).toFixed(1)} ${y(q[0]).toFixed(1)}`).join(' ');paths+=`<path d="${d}" fill="none" stroke="${RUNNER_SEGMENT_COLORS[(i-1)%RUNNER_SEGMENT_COLORS.length]}" stroke-width="8" stroke-linecap="round" stroke-linejoin="round"><title>${esc(cleanCheckpointName(cps[i-1].short||cps[i-1].name))} – ${esc(cleanCheckpointName(cps[i].short||cps[i].name))}</title></path>`}
  let markers='';cps.forEach((c,i)=>{const cx=x(c.coord?.[1]),cy=y(c.coord?.[0]),anchor=i%2?'start':'end',tx=i%2?cx+10:cx-10;markers+=`<circle cx="${cx}" cy="${cy}" r="${i===0||i===cps.length-1?8:6}" fill="#fff" stroke="#0d4c3a" stroke-width="3"/><text x="${tx}" y="${cy-9}" text-anchor="${anchor}" class="runner-map-label">${esc(cleanCheckpointName(c.short||c.name))}</text>`});
  let prevSplit=null;const legend=cps.slice(1).map((c,i)=>{const prev=cps[i],curr=runnerSplitForCheckpoint(splits,c),from=prevSplit,seconds=curr?.elapsed_seconds!=null?Number(curr.elapsed_seconds)-Number(from?.elapsed_seconds||0):null,gain=from?.place_overall&&curr?.place_overall?Number(from.place_overall)-Number(curr.place_overall):null,pace=seconds&&c.distance_km>prev.distance_km?seconds/(c.distance_km-prev.distance_km):null;prevSplit=curr||prevSplit;return `<div class="runner-segment-card"><i style="background:${RUNNER_SEGMENT_COLORS[i%RUNNER_SEGMENT_COLORS.length]}"></i><span><strong>${esc(cleanCheckpointName(prev.short||prev.name))} – ${esc(cleanCheckpointName(c.short||c.name))}</strong><small>${seconds!=null?fmtTime(seconds):'Tid saknas'}${pace?` · ${fmtPace(pace)}`:''}${gain!=null?` · ${gain>0?'+':''}${gain} platser`:''}</small></span></div>`}).join('');
...
const INFO_HELP_EXTENDED=[
  ...
  ['#genderRetentionChart','Diagrammet är banjusterat och jämför inte med den branta öppningen. För varje loppår beräknas hur stor andel av sluttiden varje delsträcka normalt tar bland fullföljande löpare. Varje löpares faktiska delsträckstid jämförs sedan med den förväntade tiden för just den delsträckan. 100 betyder förväntad prestation, 105 är cirka 5 procent snabbare än förväntat och 95 cirka 5 procent långsammare. Banorna före och från 2023 behandlas automatiskt var för sig. Endast fullföljande löpare med giltig sluttid och mellantid ingår.'],
