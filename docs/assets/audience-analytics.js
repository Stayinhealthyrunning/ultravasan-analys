...
if(typeof window!=='undefined'&&typeof document!=='undefined')(() => {
  ...
  function patchOverviewCharts(){
    ...
    renderPlacementScatter=function(){
      ...
      const W=780,H=255,p={l:54,r:20,t:18,b:42},full={minT:Math.min(...rows.map(r=>r.finish_seconds)),maxT:Math.max(...rows.map(r=>r.finish_seconds)),minP:1,maxP:Math.max(...rows.map(r=>r.overall_place))},signature=[state.raceId,selected.join(''),rows.length,full.minT,full.maxT,full.maxP].join(':');
      if(placementZoomState.signature!==signature){placementZoomState.signature=signature;placementZoomState.domain=null}
      const domain=placementZoomState.domain||full,x=t=>p.l+(t-domain.minT)*(W-p.l-p.r)/(domain.maxT-domain.minT||1),y=v=>p.t+(v-domain.minP)*(H-p.t-p.b)/(domain.maxP-domain.minP||1);let out='',marks='';
      for(let i=0;i<=4;i++){const yy=p.t+(H-p.t-p.b)*i/4,place=Math.round(domain.minP+(domain.maxP-domain.minP)*i/4);out+=svg('line',{x1:p.l,y1:yy,x2:W-p.r,y2:yy,class:'gridline'})+svg('text',{x:7,y:yy+4},String(place))}
      ...
      el.innerHTML=`<div class="chart-inline-legend">${selected.map(s=>`<span class="inline-sex-legend"><i class="${s==='M'?'male':'female'}"></i>${sexLabel(s)}</span>`).join('')}${placementZoomState.domain?'<strong>Inzoomad vy</strong>':''}</div><svg viewBox="0 0 ${W} ${H}" aria-label="Tid mot placering. Dra över ett område för att zooma."><defs><clipPath id="placementScatterClip"><rect x="${p.l}" y="${p.t}" width="${W-p.l-p.r}" height="${H-p.t-p.b}"/></clipPath></defs>${out}<g clip-path="url(#placementScatterClip)"><rect class="placement-zoom-hit" x="${p.l}" y="${p.t}" width="${W-p.l-p.r}" height="${H-p.t-p.b}"/>${marks}<rect class="placement-zoom-selection" hidden/></g></svg>`;
      if(reset)reset.disabled=!placementZoomState.domain;
      const chart=el.querySelector('svg'),hit=el.querySelector('.placement-zoom-hit'),selection=el.querySelector('.placement-zoom-selection'),bounds={left:p.l,top:p.t,right:W-p.r,bottom:H-p.b};let drag=null;
      const point=event=>{const rect=chart.getBoundingClientRect();return{x:Math.max(p.l,Math.min(W-p.r,(event.clientX-rect.left)*W/(rect.width||1))),y:Math.max(p.t,Math.min(H-p.b,(event.clientY-rect.top)*H/(rect.height||1)))}};
      ...
      const finish=event=>{if(!drag)return;const rect=drawSelection(point(event)),chartRect=chart.getBoundingClientRect(),minX=6*W/(chartRect.width||W),minY=6*H/(chartRect.height||H);cancel();if(!placementZoomDragLargeEnough(rect,minX,minY))return;const timeAt=px=>domain.minT+(px-p.l)*(domain.maxT-domain.minT)/(W-p.l-p.r),placeAt=py=>domain.minP+(py-p.t)*(domain.maxP-domain.minP)/(H-p.t-p.b);placementZoomState.domain={minT:timeAt(rect.left),maxT:timeAt(rect.right),minP:placeAt(rect.top),maxP:placeAt(rect.bottom)};renderPlacementScatter()};
