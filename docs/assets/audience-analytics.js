...
if(typeof window!=='undefined'&&typeof document!=='undefined')(() => {
  ...
  function renderSexPaceChart(el,rows,includeStart=false,relative=false,controlKey='pace'){
    ...
    const W=760,H=relative?330:280,pad={l:52,r:22,t:24,b:relative?112:66},all=points.flatMap(p=>series.map(s=>p[s]).filter(Number.isFinite)),max=relative?Math.max(120,Math.ceil(Math.max(...all)/10)*10):Math.max(2,Math.ceil(Math.max(...all)/2)*2),min=relative?Math.min(70,Math.floor(Math.min(...all)/10)*10):0,x=i=>pad.l+i*(W-pad.l-pad.r)/(points.length-1||1),y=v=>pad.t+(max-v)*(H-pad.t-pad.b)/(max-min||1);let out='';
