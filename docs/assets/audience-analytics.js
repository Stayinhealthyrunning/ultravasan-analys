...
const fixedFinishBins=(times,step=900)=>{const values=(times||[]).map(Number).filter(Number.isFinite).sort((a,b)=>a-b);if(!values.length)return{start:null,step,bins:[]};const start=Math.floor(values[0]/step)*step,count=Math.floor((values.at(-1)-start)/step)+1,bins=Array.from({length:count},(_,i)=>({from:start+i*step,to:start+(i+1)*step,count:0}));for(const value of values)bins[Math.floor((value-start)/step)].count++;return{start,step,bins}};
const audienceFinishBinTime=seconds=>{const value=Math.max(0,Math.floor(Number(seconds)||0)),hours=Math.floor(value/3600),minutes=Math.floor(value%3600/60);return `${hours}:${String(minutes).padStart(2,'0')}`};
const finishHistogramGeometry=(binCount,width=1000,left=45,right=16)=>{const cellWidth=(width-left-right)/Math.max(1,Number(binCount)||1),barWidth=Math.max(2,Math.min(28,cellWidth*.78));return{cellWidth,barWidth,barOffset:(cellWidth-barWidth)/2}};
const normalizePlacementZoomRect=(start,current,bounds)=>{const clampX=value=>Math.max(bounds.left,Math.min(bounds.right,Number(value))),clampY=value=>Math.max(bounds.top,Math.min(bounds.bottom,Number(value))),x1=clampX(start.x),x2=clampX(current.x),y1=clampY(start.y),y2=clampY(current.y),left=Math.min(x1,x2),top=Math.min(y1,y2),right=Math.max(x1,x2),bottom=Math.max(y1,y2);return{left,top,right,bottom,width:right-left,height:bottom-top}};
...
if(typeof module!=='undefined'&&module.exports)module.exports={audienceRaceFamily,selectAudienceRace,aggregateSpeedKmh,speedIndex,relativeToplistWidth,visibleCountBarHeight,fixedFinishBins,finishBinTime:audienceFinishBinTime,finishHistogramGeometry,normalizePlacementZoomRect,placementZoomDragLargeEnough,placementPlotBounds,selectedClubProfiles,mergedClubFinishers,fastestTenPercentAverage};
...
if(typeof window!=='undefined'&&typeof document!=='undefined')(() => {
  ...
  const advanced={ready:false,clubMetric:'largest',classSelection:[],classSelectionInitialized:false,clubSelection:[],clubKeyByResult:new Map(),clubDisplay:new Map(),resultById:new Map(),splitsByResult:null,splitEvidence:new Set(),smIndex:new Map(),classIndexMode:'dominance',classHeatUnit:'pace',classHeatStatistic:'median',yearTimer:null,currentClubStats:[],clubSearchReady:false,classEvolutionController:null,classEvolutionCache:new Map(),classEvolutionModel:null,classEvolutionKey:'',historyReady:false,historyRestoring:false,historyRestoreQueued:false,historyListenerReady:false};
  ...
  const splitRowsForResults=rows=>window.UltravasanDataIndex.splitsForResults(state.data,rows);
  ...
  async function restoreHistoryState(){
    if(advanced.historyRestoring){advanced.historyRestoreQueued=true;return}
    ...
    try{
      ...
      if(advanced.historyRestoreQueued){
        advanced.historyRestoreQueued=false;
        queueMicrotask(()=>restoreHistoryState());
      }
    }
  }
  ...
  function patchOverviewCharts(){
    renderHistogram=function(){
      ...
      const times=rows.map(r=>r.finish_seconds),grouped=fixedFinishBins(times),bins=grouped.bins,counts=bins.map(()=>({M:0,F:0}));rows.forEach(r=>counts[Math.min(bins.length-1,Math.floor((r.finish_seconds-grouped.start)/grouped.step))][sexKey(r)]++);
      const series=visibleSexes('histogram'),W=1000,H=280,p={l:45,r:16,t:18,b:44},{cellWidth:cw,barWidth,barOffset}=finishHistogramGeometry(bins.length,W,p.l,p.r),ymax=Math.max(...counts.map(c=>series.reduce((sum,s)=>sum+c[s],0)),1);let out='';
      ...
      counts.forEach((c,i)=>{const x=p.l+i*cw+barOffset,bin=bins[i];let base=H-p.b;series.forEach(k=>{const color=k==='M'?COLORS.male:COLORS.female;if(!c[k])return;const h=(H-p.t-p.b)*c[k]/ymax;base-=h;const tip=`${sexLabel(k)}: ${c[k]} · ${audienceFinishBinTime(bin.from)}–${audienceFinishBinTime(bin.to)}`;out+=`<rect x="${x}" y="${base}" width="${barWidth}" height="${h}" rx="2" fill="${color}" class="sex-bar" tabindex="0" role="img" aria-label="${esc(tip)}"><title>${esc(tip)}</title></rect>`});if(i%Math.max(1,Math.ceil(bins.length/7))===0||i===bins.length-1)out+=svg('text',{x:p.l+(i+.5)*cw,y:H-18,'text-anchor':'middle'},audienceFinishBinTime(bin.from))});
      out+=svg('line',{x1:p.l,y1:H-p.b,x2:W-p.r,y2:H-p.b,class:'axis'});el.innerHTML=`<svg viewBox="0 0 ${W} ${H}">${out}</svg>`;
      const labels=[];if(vis.M)labels.push(`Män ${fmtTime(safeMed(rows.filter(r=>sexKey(r)==='M').map(r=>r.finish_seconds)))}`);if(vis.F)labels.push(`Kvinnor ${fmtTime(safeMed(rows.filter(r=>sexKey(r)==='F').map(r=>r.finish_seconds)))}`);document.querySelector('#distributionLabel').textContent=`15-minutersintervall från ${audienceFinishBinTime(grouped.start)} · median: ${labels.join(' · ')}`;
    };
    ...
    renderPlacementScatter=function(){
      stopPlacementZoomDrag();const el=document.querySelector('#placementScatter'),reset=document.querySelector('#placementZoomReset'),selected=visibleSexes('placement'),rows=filterRowsForSexControl(state.filtered.filter(r=>r.finish_seconds&&r.overall_place),'placement');
      if(!selected.length){el.innerHTML='<div class="empty">Välj minst ett kön i placeringsmotorn</div>';if(reset)reset.disabled=true;return}
      if(rows.length<2){el.innerHTML='<div class="empty">Minst två placerade löpare behövs</div>';if(reset)reset.disabled=true;return}
      const W=780,H=255,p={l:54,r:20,t:18,b:42},{plot,data:dataBounds}=placementPlotBounds(W,H,p),full={minT:Math.min(...rows.map(r=>r.finish_seconds)),maxT:Math.max(...rows.map(r=>r.finish_seconds)),minP:1,maxP:Math.max(...rows.map(r=>r.overall_place))},signature=[state.raceId,selected.join(''),rows.length,full.minT,full.maxT,full.maxP].join(':');
      if(placementZoomState.signature!==signature){placementZoomState.signature=signature;placementZoomState.domain=null}
      const domain=placementZoomState.domain||full,x=t=>dataBounds.left+(t-domain.minT)*(dataBounds.right-dataBounds.left)/(domain.maxT-domain.minT||1),y=v=>dataBounds.top+(v-domain.minP)*(dataBounds.bottom-dataBounds.top)/(domain.maxP-domain.minP||1);let out='',marks='';
      for(let i=0;i<=4;i++){const yy=dataBounds.top+(dataBounds.bottom-dataBounds.top)*i/4,place=Math.round(domain.minP+(domain.maxP-domain.minP)*i/4);out+=svg('line',{x1:plot.left,y1:yy,x2:plot.right,y2:yy,class:'gridline'})+svg('text',{x:7,y:yy+4},String(place))}
      for(let i=0;i<=4;i++){const tt=domain.minT+(domain.maxT-domain.minT)*i/4,xx=x(tt);out+=svg('text',{x:xx,y:H-12,'text-anchor':'middle'},fmtTime(tt).slice(0,-3))}
      rows.filter(r=>r.finish_seconds>=domain.minT&&r.finish_seconds<=domain.maxT&&r.overall_place>=domain.minP&&r.overall_place<=domain.maxP).forEach(r=>{const sx=sexKey(r),color=sx==='M'?COLORS.male:COLORS.female,cx=x(r.finish_seconds),cy=y(r.overall_place),title=`${r.name_as_published} · ${sexLabel(sx)} · ${fmtTime(r.finish_seconds)} · plats ${r.overall_place}`,common=`class="placement-scatter-point" tabindex="0" role="img" aria-label="${esc(title)}"`;marks+=sx==='F'?`<path ${common} d="M${cx} ${cy-5} L${cx+5} ${cy} L${cx} ${cy+5} L${cx-5} ${cy} Z" fill="${color}" opacity=".72"><title>${esc(title)}</title></path>`:`<circle ${common} cx="${cx}" cy="${cy}" r="4.2" fill="${color}" opacity=".68"><title>${esc(title)}</title></circle>`});
      el.innerHTML=`<div class="chart-inline-legend">${selected.map(s=>`<span class="inline-sex-legend"><i class="${s==='M'?'male':'female'}"></i>${sexLabel(s)}</span>`).join('')}${placementZoomState.domain?'<strong>Inzoomad vy</strong>':''}</div><svg viewBox="0 0 ${W} ${H}" aria-label="Tid mot placering. Dra över ett område för att zooma."><defs><clipPath id="placementScatterClip"><rect x="${plot.left}" y="${plot.top}" width="${plot.right-plot.left}" height="${plot.bottom-plot.top}"/></clipPath></defs>${out}<g clip-path="url(#placementScatterClip)"><rect class="placement-zoom-hit" x="${dataBounds.left}" y="${dataBounds.top}" width="${dataBounds.right-dataBounds.left}" height="${dataBounds.bottom-dataBounds.top}"/>${marks}<rect class="placement-zoom-selection" hidden/></g></svg>`;
      if(reset)reset.disabled=!placementZoomState.domain;
      const chart=el.querySelector('svg'),hit=el.querySelector('.placement-zoom-hit'),selection=el.querySelector('.placement-zoom-selection'),bounds=dataBounds;let drag=null;
      const point=event=>{const rect=chart.getBoundingClientRect();return{x:Math.max(dataBounds.left,Math.min(dataBounds.right,(event.clientX-rect.left)*W/(rect.width||1))),y:Math.max(dataBounds.top,Math.min(dataBounds.bottom,(event.clientY-rect.top)*H/(rect.height||1)))}};
      const drawSelection=current=>{const rect=normalizePlacementZoomRect(drag,current,bounds);selection.setAttribute('x',rect.left);selection.setAttribute('y',rect.top);selection.setAttribute('width',rect.width);selection.setAttribute('height',rect.height);return rect},hideSelection=()=>selection.setAttribute('hidden',''),cancel=()=>{if(drag&&hit.hasPointerCapture?.(drag.pointerId))hit.releasePointerCapture(drag.pointerId);drag=null;hideSelection();document.removeEventListener('keydown',onKeyDown)},onKeyDown=event=>{if(event.key==='Escape'&&drag){event.preventDefault();cancel()}};
      cancelPlacementZoomDrag=cancel;
      hit?.addEventListener('pointerdown',event=>{if(event.button>0)return;event.preventDefault();drag={...point(event),pointerId:event.pointerId};hit.setPointerCapture?.(event.pointerId);selection.removeAttribute('hidden');drawSelection(drag);document.addEventListener('keydown',onKeyDown)});
      hit?.addEventListener('pointermove',event=>{if(drag)drawSelection(point(event))});
      const finish=event=>{if(!drag)return;const rect=drawSelection(point(event)),chartRect=chart.getBoundingClientRect(),minX=6*W/(chartRect.width||W),minY=6*H/(chartRect.height||H);cancel();if(!placementZoomDragLargeEnough(rect,minX,minY))return;const timeAt=px=>domain.minT+(px-dataBounds.left)*(domain.maxT-domain.minT)/(dataBounds.right-dataBounds.left),placeAt=py=>domain.minP+(py-dataBounds.top)*(domain.maxP-domain.minP)/(dataBounds.bottom-dataBounds.top);placementZoomState.domain={minT:timeAt(rect.left),maxT:timeAt(rect.right),minP:placeAt(rect.top),maxP:placeAt(rect.bottom)};renderPlacementScatter()};
      hit?.addEventListener('pointerup',finish);hit?.addEventListener('pointercancel',cancel);hit?.addEventListener('dblclick',resetPlacementZoom);
    };
  ...
  function renderSexPaceChart(el,rows,includeStart=false,relative=false,controlKey='pace'){
    ...
    rows=filterRowsForSexControl(rows,controlKey);const race=state.data.races.find(r=>r.id===state.raceId),eligible=relative?rows.filter(isFinished):rows,cpBySeq=new Map(cpList().map(c=>[Number(c.sequence_no),Number(c.distance_km)])),groups=new Map();
    splitRowsForResults(eligible).filter(s=>Number(s.pace_seconds_per_km)>0).forEach(s=>{const speed=3600/Number(s.pace_seconds_per_km),seq=Number(s.sequence_no),distance=Number(s.distance_km)-Number(cpBySeq.get(seq-1)??0),seconds=Number(s.segment_seconds);if(!Number.isFinite(speed)||speed<=0||speed>30)return;if(!groups.has(seq))groups.set(seq,{name:s.checkpoint_name,M:[],F:[]});const sx=resultSex(s.result_id);if(!groups.get(seq)[sx])return;groups.get(seq)[sx].push(relative?{distanceKm:distance,seconds}:speed)});
    ...
    const overviewPace=controlKey==='pace'&&!relative,W=overviewPace?420:760,H=relative?330:300,pad={l:overviewPace?48:58,r:overviewPace?14:24,t:24,b:relative?112:overviewPace?88:82},all=points.flatMap(p=>series.map(s=>p[s]).filter(Number.isFinite)),rawMin=Math.min(...all),rawMax=Math.max(...all),max=relative?Math.max(120,Math.ceil(rawMax/10)*10):rawMax,min=relative?Math.min(70,Math.floor(rawMin/10)*10):rawMin,x=i=>pad.l+i*(W-pad.l-pad.r)/(points.length-1||1),y=v=>paceMode?pad.t+(v-min)*(H-pad.t-pad.b)/(max-min||1):pad.t+(max-v)*(H-pad.t-pad.b)/(max-min||1);let out='';
    const axisValues=relative
      ?[max,(max+100)/2,100,(min+100)/2,min].map(value=>Math.round(value)).filter((value,index,array)=>array.indexOf(value)===index).sort((a,b)=>b-a)
      :Array.from({length:5},(_,i)=>paceMode?min+(max-min)*i/4:max-(max-min)*i/4);
    axisValues.forEach(v=>{const yy=y(v),label=relative?`${Math.round(v)}`:formatSpeedAxis(v);out+=svg('line',{x1:pad.l,y1:yy,x2:W-pad.r,y2:yy,class:'gridline'})+svg('text',{x:5,y:yy+4},label)});
    if(relative&&min<=100&&max>=100)out+=svg('line',{x1:pad.l,y1:y(100),x2:W-pad.r,y2:y(100),class:'pace-reference-line'});
    ...
    points.forEach((d,i)=>{const full=String(d.name||'').replace('Mora mål','Mora'),parts=full.split(/\s+[–-]\s+/),label=overviewPace&&parts.length===2?compactSegmentLabel({from:parts[0],to:parts[1]}):full,px=x(i),py=H-(relative?54:29);out+=`<text x="${px}" y="${py}" text-anchor="middle" transform="rotate(-34 ${px} ${py})" class="${relative?'pace-segment-label':''}" aria-label="${esc(full)}"><title>${esc(full)}</title>${esc(label)}</text>`});
  ...
  function patchNerdCharts(){
    renderPercentiles=function(){
      const el=document.querySelector('#percentileLadder'),selected=visibleSexes('percentile');if(!el)return
      const rows=state.filtered.filter(isFinished),all=rows.map(r=>Number(r.finish_seconds)).filter(Number.isFinite),m=rows.filter(r=>sexKey(r)==='M').map(r=>Number(r.finish_seconds)).filter(Number.isFinite),f=rows.filter(r=>sexKey(r)==='F').map(r=>Number(r.finish_seconds)).filter(Number.isFinite);if(all.length<2){el.innerHTML='<div class="empty">Fler sluttider krävs</div>';return}
      const levels=[[.10,'10 % i mål'],[.25,'25 % i mål'],[.50,'50 % i mål · median'],[.75,'75 % i mål'],[.90,'90 % i mål']];el.innerHTML=`<div class="percentile-overview"><div><strong>${rows.length.toLocaleString('sv-SE')}</strong><span>fullföljande i aktuellt urval</span></div><p>Varje nivå visar tidpunkten då motsvarande andel av de fullföljande hade gått i mål. Huvudvärdet följer huvudsidans aktiva filter. Könsknapparna styr endast de sekundära könsvärdena.</p></div><div class="percentile-grid">${levels.map(([q,label])=>`<article class="percentile-tile" data-finish-share="${Math.round(q*100)}"><span>${label}</span><div class="percentile-primary"><small>Aktuellt urval</small><strong>${fmtTime(quantile(all,q))}</strong></div><div class="percentile-sex-values">${selected.includes('M')?`<div class="male"><small>Män</small><strong>${fmtTime(quantile(m,q))}</strong></div>`:''}${selected.includes('F')?`<div class="female"><small>Kvinnor</small><strong>${fmtTime(quantile(f,q))}</strong></div>`:''}</div></article>`).join('')}</div>`;
    };
  }
  ...
  function renderClassCompare(stats){
    const rows=stats.filter(x=>advanced.classSelection.includes(x.key)),el=document.querySelector('#classCompareChart'),cps=cpList().filter(c=>c.sequence_no>0),pairs=segmentPairs(cps);if(!rows.length){el.innerHTML='<div class="empty class-empty-state">Inga klasser valda.</div>';return}
  ...
  function renderClassHistory(){
    const el=document.querySelector('#classHistoryChart'),all=filteredAcrossYears({ignoreClass:true}),years=familyRaces().slice().sort((a,b)=>a.year-b.year),classes=advanced.classSelection.slice().sort(compareClasses);if(!classes.length){el.innerHTML='<div class="empty class-empty-state">Inga klasser valda.</div>';return}
    advanced.classHistoryMode=advanced.classHistoryMode==='participation'?'participation':'median';
    const data=classes.map((k,ki)=>({k,color:CLASS_COLORS[ki%CLASS_COLORS.length],pts:years.map(r=>{const rr=all.filter(x=>x.race_id===r.id&&normClass(x.age_class)===k),st=rr.filter(isStarter),fin=rr.filter(isFinished),dnf=st.filter(isDnf);return{year:r.year,med:safeMed(fin.map(x=>x.finish_seconds)),starters:st.length,dnf:dnf.length,dnfRate:st.length?dnf.length/st.length*100:null}})})),mode=advanced.classHistoryMode,W=1000,H=430,p={l:62,r:28,t:50,b:48},plotHeight=H-p.t-p.b,plotWidth=W-p.l-p.r,x=i=>p.l+i*plotWidth/(years.length-1||1);let out='',marks='';
    const toolbar=`<div class="class-history-toolbar"><div class="class-history-modes" role="tablist" aria-label="Välj vy för klasshistorik"><button type="button" class="${mode==='median'?'active':''}" data-class-history-mode="median" aria-pressed="${mode==='median'}">Mediantid</button><button type="button" class="${mode==='participation'?'active':''}" data-class-history-mode="participation" aria-pressed="${mode==='participation'}">Deltagande</button></div><p class="class-history-subcopy">${mode==='median'?'Hur har klassernas typiska sluttid förändrats?':'Hur många startade – och hur stor del bröt loppet?'}</p></div>`;
    const legend=`<div class="class-chart-legend">${data.map(s=>`<span><i style="background:${s.color}"></i>${esc(s.k)}</span>`).join('')}</div>`;
    if(mode==='median'){
      const allTimes=data.flatMap(s=>s.pts.map(d=>d.med).filter(Number.isFinite));if(!allTimes.length){el.innerHTML=toolbar+'<div class="empty">Historik saknas för valda klasser.</div>';return}
      const lo=Math.floor(Math.min(...allTimes)/1800)*1800,hi=Math.ceil(Math.max(...allTimes)/1800)*1800,y=v=>p.t+(hi-v)*plotHeight/(hi-lo||1);
      for(let i=0;i<=4;i++){const v=hi-(hi-lo)*i/4,yy=y(v);out+=svg('line',{x1:p.l,y1:yy,x2:W-p.r,y2:yy,class:'gridline'})+svg('text',{x:p.l-10,y:yy+4,'text-anchor':'end'},fmtTime(v).slice(0,-3))}
      out+=svg('text',{x:17,y:(p.t+H-p.b)/2,'text-anchor':'middle',transform:`rotate(-90 17 ${(p.t+H-p.b)/2})`,class:'class-history-axis-title'},'Median sluttid')+svg('line',{x1:p.l,y1:H-p.b,x2:W-p.r,y2:H-p.b,class:'axis'});
      data.forEach(s=>{const valid=s.pts.map((d,i)=>({...d,i})).filter(d=>Number.isFinite(d.med));descriptiveHistoryRuns(valid,years).filter(run=>run.length>1).forEach(run=>{marks+=`<path class="class-history-line" d="${run.map((d,j)=>`${j?'L':'M'}${x(d.i)} ${y(d.med)}`).join(' ')}" fill="none" stroke="${s.color}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>`});valid.forEach(d=>{const tip=`${s.k} ${d.year} · median ${fmtTime(d.med)}`,px=x(d.i),py=y(d.med);marks+=`<circle cx="${px}" cy="${py}" r="4.5" fill="${s.color}"/><circle class="class-history-hit" cx="${px}" cy="${py}" r="9" fill="transparent" tabindex="0" role="img" aria-label="${esc(tip)}" data-chart-tip="${esc(tip)}" data-chart-x="${(px/W*100).toFixed(2)}" data-chart-y="${(py/H*100).toFixed(2)}"><title>${esc(tip)}</title></circle>`})});
    }else{
      const rawMax=Math.max(1,...data.flatMap(s=>s.pts.map(d=>d.starters))),roughStep=rawMax/5,power=10**Math.floor(Math.log10(roughStep||1)),unit=roughStep/power,countStep=(unit<=1?1:unit<=2?2:unit<=5?5:10)*power,maxN=Math.ceil(rawMax/countStep)*countStep,y=v=>p.t+plotHeight-v/maxN*plotHeight,step=plotWidth/(years.length-1||1),groupWidth=Math.min(94,step*.82),barWidth=Math.max(8,Math.min(18,groupWidth/(classes.length||1)));
      for(let v=0;v<=maxN+.001;v+=countStep){const yy=y(v);out+=svg('line',{x1:p.l,y1:yy,x2:W-p.r,y2:yy,class:'gridline'})+svg('text',{x:p.l-10,y:yy+4,'text-anchor':'end'},String(Math.round(v)))}
      out+=svg('text',{x:17,y:(p.t+H-p.b)/2,'text-anchor':'middle',transform:`rotate(-90 17 ${(p.t+H-p.b)/2})`,class:'class-history-axis-title'},'Antal startande')+svg('line',{x1:p.l,y1:H-p.b,x2:W-p.r,y2:H-p.b,class:'axis'});
      data.forEach((s,si)=>s.pts.forEach((d,i)=>{if(!d.starters)return;const left=x(i)-classes.length*barWidth/2+si*barWidth,totalH=d.starters/maxN*plotHeight,dnfRaw=d.dnf/maxN*plotHeight,dnfH=d.dnf?Math.min(totalH,Math.max(2,dnfRaw)):0,baseH=Math.max(0,totalH-dnfH),tip=`${s.k} ${d.year} · ${d.starters} startande · DNF ${d.dnf}${Number.isFinite(d.dnfRate)?` (${d.dnfRate.toFixed(1)} %)`:''}`;if(baseH)marks+=`<rect class="class-history-bar starters" x="${left}" y="${H-p.b-baseH}" width="${Math.max(1,barWidth-1.2)}" height="${baseH}" fill="${s.color}" fill-opacity=".28"/>`;if(dnfH)marks+=`<rect class="class-history-bar dnf" x="${left}" y="${H-p.b-totalH}" width="${Math.max(1,barWidth-1.2)}" height="${dnfH}" fill="${s.color}" fill-opacity=".9"/>`;marks+=`<rect class="class-history-hit" x="${left}" y="${H-p.b-totalH}" width="${Math.max(1,barWidth-1.2)}" height="${Math.max(3,totalH)}" fill="transparent" tabindex="0" role="img" aria-label="${esc(tip)}" data-chart-tip="${esc(tip)}" data-chart-x="${((left+barWidth/2)/W*100).toFixed(2)}" data-chart-y="${((H-p.b-totalH)/H*100).toFixed(2)}"><title>${esc(tip)}</title></rect>`}));
    }
    years.forEach((r,i)=>out+=svg('text',{x:x(i),y:H-16,'text-anchor':'middle'},String(r.year)));
    const key=mode==='median'?'<div class="class-history-key"><span><i class="median"></i>Median sluttid</span></div>':'<div class="class-history-key"><span><i class="starters"></i>Fullföljde eller pågående</span><span><i class="dnf"></i>DNF · mörk del av stapeln</span></div>';
    const note=mode==='median'?'En linje per vald klass. Kalenderår utan lopp lämnas som ett avbrott.':'Stapelhöjden är alla startande. Den mörka delen är DNF; exakt antal och andel visas när du hovrar eller fokuserar.';
    el.innerHTML=`${toolbar}${legend}${key}<div class="interactive-chart-tooltip" role="status" hidden></div><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${mode==='median'?'Klasshistorik: median sluttid över åren':'Klasshistorik: startande och DNF över åren'}">${out}${marks}</svg><p class="class-history-note">${note}</p>`;
    el.querySelectorAll('[data-class-history-mode]').forEach(button=>button.addEventListener('click',()=>{const next=button.dataset.classHistoryMode;if(next===advanced.classHistoryMode)return;advanced.classHistoryMode=next;renderClassHistory()}));wireChartTooltips(el);
  }
  ...
  function renderClassEvolution(){
    ...
    if(!model){model=api.aggregateClassHistory({races,results:filteredAcrossYears({ignoreClass:true}),getClass:r=>normClass(r.age_class),getGender:sexKey,isStarter,isFinished,comparisonKeyForRace:historyComparisonKey,courseVersionForRace:race=>window.HistoryIntelligence?.courseVersionId?.(race)||null});advanced.classEvolutionCache.set(cacheKey,model)}
  ...
  function clubHistoryImprovement(key){
    const selectedRace=state.data.races.find(r=>String(r.id)===String(state.raceId)),comparisonKey=historyComparisonKey(selectedRace);
    if(!comparisonKey)return null;
    const races=familyRaces().slice().sort((a,b)=>a.year-b.year).filter(r=>historyComparisonKey(r)===comparisonKey);
    const rows=familyResults().filter(r=>advanced.clubKeyByResult.get(r.id)===key&&isFinished(r));
    const pts=races.map(race=>{const values=rows.filter(row=>row.race_id===race.id).map(row=>advanced.smIndex.get(row.id)).filter(Number.isFinite);return values.length>=3?{year:race.year,med:safeMed(values)}:null}).filter(Boolean);
    if(pts.length<2)return null;
    return {value:pts.at(-1).med-pts[0].med,fromYear:pts[0].year,toYear:pts.at(-1).year,comparisonKey};
  }
  function renderClubWorld(){
    ...
    const selected=selectedClubProfiles(stats,advanced.clubSelection);
    if(selected.length){const active=selected.find(x=>x.key===selectedKey)||selected.at(-1);renderClubProfiles(selected,stats);renderClubDna(active,stats)}
    else{if(profile)profile.innerHTML='<div class="empty panel club-profile-placeholder"><strong>Sök och välj en klubb eller ort</strong><span>Alla valda klubbar och orter visas här i urvalsordning.</span></div>';if(dna)dna.innerHTML='<div class="empty">Välj en klubb/ort ovan för att visa klubbens profil.</div>';if(history)history.innerHTML='<div class="empty">Välj en klubb/ort ovan för att visa historiken.</div>'}
  }
  ...
  function renderClubProfiles(clubs){
    const el=document.querySelector('#clubProfile'),fastest=mergedClubFinishers(clubs,20);
    const profiles=clubs.map(c=>{
      const sample=c.starters<5?'<span class="small-sample">Litet underlag</span> ':'';
      return `<article class="club-profile-summary" data-club-profile="${esc(c.key)}"><div><h3>${esc(c.name)}</h3><p>${sample}${c.starters} startande · ${c.finishers} fullföljande</p></div><div class="club-profile-kpis"><span>Startande<strong>${c.starters}</strong></span><span>Fullföljande<strong>${c.finishers}</strong></span><span>Median<strong>${fmtTime(c.median)}</strong></span><span>I mål<strong>${c.rate} %</strong></span><span>SM-index<strong>${c.medianIndex?.toFixed(0)||'–'}</strong></span><span>Klasser<strong>${c.classes}</strong></span></div></article>`
    }).join('');
    const runners=fastest.map((entry,i)=>`<button data-id="${entry.result.id}"><b>${i+1}</b><span><strong>${esc(entry.result.name_as_published)}</strong><small>${esc(entry.clubNames.join(' · '))}</small></span><em>${fmtTime(entry.result.finish_seconds)}</em></button>`).join('')||'<span class="selection-empty">Inga giltiga sluttider i urvalet.</span>';
    el.innerHTML=`<article class="club-profile-hero club-profile-multi panel"><section class="club-profile-overview"><p class="eyebrow">VALDA KLUBBAR/ORTER</p><div class="club-profile-list" aria-label="Profiler för valda klubbar och orter">${profiles}</div></section><section class="club-top-runners club-top-runners-merged"><p class="eyebrow">SNABBASTE I URVALET</p><small>Gemensam lista · upp till 20 fullföljande</small><div class="club-top-runners-list">${runners}</div></section></article>`;
    el.querySelectorAll('button[data-id]').forEach(b=>b.onclick=()=>openRunner(Number(b.dataset.id)))
  }
