...
if(typeof module!=='undefined'&&module.exports)module.exports={audienceRaceFamily,selectAudienceRace,aggregateSpeedKmh,speedIndex,relativeToplistWidth,visibleCountBarHeight,fixedFinishBins};
...
if(typeof window!=='undefined'&&typeof document!=='undefined')(() => {
  ...
  function patchOverviewCharts(){
    renderHistogram=function(){
      ...
      counts.forEach((c,i)=>{const x=p.l+i*cw+2,bin=bins[i];let base=H-p.b;series.forEach(k=>{const color=k==='M'?COLORS.male:COLORS.female;if(!c[k])return;const h=(H-p.t-p.b)*c[k]/ymax;base-=h;const tip=`${sexLabel(k)}: ${c[k]} · ${fmtTime(bin.from)}–${fmtTime(bin.to)}`;out+=`<rect x="${x}" y="${base}" width="${Math.max(2,cw-4)}" height="${h}" rx="2" fill="${color}" class="sex-bar" tabindex="0" role="img" aria-label="${esc(tip)}"><title>${esc(tip)}</title></rect>`});if(i%Math.max(1,Math.ceil(bins.length/7))===0||i===bins.length-1)out+=svg('text',{x,y:H-18},fmtTime(bin.from).slice(0,-3))});
      out+=svg('line',{x1:p.l,y1:H-p.b,x2:W-p.r,y2:H-p.b,class:'axis'});el.innerHTML=`<svg viewBox="0 0 ${W} ${H}" style="min-width:${W}px">${out}</svg>`;
      const labels=[];if(vis.M)labels.push(`Män ${fmtTime(safeMed(rows.filter(r=>sexKey(r)==='M').map(r=>r.finish_seconds)))}`);if(vis.F)labels.push(`Kvinnor ${fmtTime(safeMed(rows.filter(r=>sexKey(r)==='F').map(r=>r.finish_seconds)))}`);document.querySelector('#distributionLabel').textContent=`30-minutersintervall från ${fmtTime(grouped.start)} · median: ${labels.join(' · ')}`;
    };
