...
(() => {
  ...
  function patchOverviewCharts(){
    renderHistogram=function(){
      const rows=state.filtered.filter(isFinished),el=document.querySelector('#histogram');if(!rows.length){el.innerHTML='<div class="empty">Inga sluttider i urvalet</div>';return}
      ...
      const counts=Array.from({length:bins},()=>({M:0,F:0,U:0}));rows.forEach(r=>counts[Math.min(bins-1,Math.floor((r.finish_seconds-min)/step))][sexKey(r)]++);
      const W=680,H=280,p={l:45,r:16,t:18,b:44},cw=(W-p.l-p.r)/bins,ymax=Math.max(...counts.map(c=>c.M+c.F+c.U),1);let out='';
      ...
      counts.forEach((c,i)=>{const x=p.l+i*cw+2;let base=H-p.b;[['M',COLORS.male],['F',COLORS.female],['U',COLORS.unknown]].forEach(([k,color])=>{if(!c[k])return;const h=(H-p.t-p.b)*c[k]/ymax;base-=h;out+=`<rect x="${x}" y="${base}" width="${Math.max(2,cw-4)}" height="${h}" rx="2" fill="${color}" class="sex-bar"><title>${sexLabel(k)}: ${c[k]} · ${fmtTime(min+i*step)}–${fmtTime(min+(i+1)*step)}</title></rect>`});if(i%Math.ceil(bins/6)===0)out+=svg('text',{x,y:H-18},`${Math.floor((min+i*step)/3600)} h`)});
      out+=svg('line',{x1:p.l,y1:H-p.b,x2:W-p.r,y2:H-p.b,class:'axis'});el.innerHTML=`<svg viewBox="0 0 ${W} ${H}">${out}</svg>`;
      const mt=safeMed(rows.filter(r=>sexKey(r)==='M').map(r=>r.finish_seconds)),ft=safeMed(rows.filter(r=>sexKey(r)==='F').map(r=>r.finish_seconds));document.querySelector('#distributionLabel').textContent=`Median: män ${fmtTime(mt)} · kvinnor ${fmtTime(ft)}`;
    };
    ...
    renderSegmentHeatmap=function(){
      const el=document.querySelector('#segmentHeatmap'),ids=new Set(state.filtered.map(r=>r.id)),groups=new Map();state.data.splits.filter(s=>ids.has(s.result_id)&&s.pace_seconds_per_km&&s.sequence_no>0).forEach(s=>{if(!groups.has(s.sequence_no))groups.set(s.sequence_no,{name:s.checkpoint_name,M:[],F:[]});const k=resultSex(s.result_id);if(groups.get(s.sequence_no)[k])groups.get(s.sequence_no)[k].push(Number(s.pace_seconds_per_km))});
      ...
      el.innerHTML=cells.map(c=>`<div class="segment-cell sex-segment-cell"><span>${esc(c.label)}</span><div><b class="male">M</b><strong>${fmtPace(safeMed(c.M)).replace(' /km','')}</strong></div><div><b class="female">K</b><strong>${fmtPace(safeMed(c.F)).replace(' /km','')}</strong></div><small>${c.M.length+c.F.length} registrerade tider</small></div>`).join('');
    };
    renderYearTrend=renderSexYearTrend;
  }

  function renderSexPaceChart(el,rows,includeStart=false,relative=false){
    if(!el)return;const ids=new Set(rows.map(r=>r.id)),groups=new Map();state.data.splits.filter(s=>ids.has(s.result_id)&&Number(s.pace_seconds_per_km)>0).forEach(s=>{const speed=3600/Number(s.pace_seconds_per_km);if(!Number.isFinite(speed)||speed<=0||speed>30)return;if(!groups.has(s.sequence_no))groups.set(s.sequence_no,{name:s.checkpoint_name,M:[],F:[]});const sx=resultSex(s.result_id);if(groups.get(s.sequence_no)[sx])groups.get(s.sequence_no)[sx].push(speed)});
    ...
    if(relative){['M','F'].forEach(s=>{const base=points.find(p=>p[s])?.[s];points.forEach(p=>{p[s]=base&&p[s]?p[s]/base*100:null})})}
    if(includeStart)points.unshift({seq:0,name:'Sälen',M:0,F:0});
    const series=['M','F'].filter(s=>points.some(p=>Number.isFinite(p[s]))),W=760,H=280,pad={l:52,r:22,t:24,b:66},all=points.flatMap(p=>series.map(s=>p[s]).filter(Number.isFinite)),max=relative?Math.max(120,Math.ceil(Math.max(...all)/10)*10):Math.max(2,Math.ceil(Math.max(...all)/2)*2),min=relative?Math.min(50,Math.floor(Math.min(...all)/10)*10):0,x=i=>pad.l+i*(W-pad.l-pad.r)/(points.length-1||1),y=v=>pad.t+(max-v)*(H-pad.t-pad.b)/(max-min||1);let out='';
  ...
  function renderGenderDnf(){
    const el=document.querySelector('#dnfFunnel'),rows=state.filtered;if(!el)return;if(!rows.length){el.innerHTML='<div class="empty">Inget underlag</div>';return}
    ...
    const cps=cpList(),segments=new Map();dnf.forEach(r=>{const arr=(advanced.splitsByResult.get(r.id)||[]).filter(s=>Number.isFinite(Number(s.sequence_no))).sort((a,b)=>a.sequence_no-b.sequence_no),last=arr.at(-1);let key='unknown',label='Plats saknas i källdatan',order=999;if(last){const from=cps.find(c=>c.sequence_no===last.sequence_no),next=cps.find(c=>c.sequence_no>last.sequence_no);label=`${(from?.name||last.checkpoint_name||'Senaste kontroll').replace('Mora mål','Mora')} → ${(next?.name||'Mora').replace('Mora mål','Mora')}`;key=`${last.sequence_no}-${next?.sequence_no??999}`;order=Number(last.sequence_no)}if(!segments.has(key))segments.set(key,{label,order,M:0,F:0,U:0});segments.get(key)[sexKey(r)]++});
    const data=[...segments.values()].sort((a,b)=>a.order-b.order),max=Math.max(...data.map(x=>x.M+x.F+x.U),1),m=dnf.filter(r=>sexKey(r)==='M').length,f=dnf.filter(r=>sexKey(r)==='F').length;
    el.innerHTML=`<div class="dnf-summary"><strong>${dnf.length.toLocaleString('sv-SE')} DNF av ${starters.length.toLocaleString('sv-SE')} startande</strong><span>Män ${m} · kvinnor ${f} · ${dns.length} DNS räknas inte som startande.</span></div><div class="dnf-segments sex-dnf">${data.map(x=>{const total=x.M+x.F+x.U,w=total/max*100,wm=total?x.M/total*100:0,wf=total?x.F/total*100:0;return `<div class="dnf-segment-row"><span>${esc(x.label)}</span><div class="stack-track" style="width:${Math.max(5,w)}%"><i class="male" style="width:${wm}%"></i><i class="female" style="width:${wf}%"></i></div><strong>${total}</strong><small><b class="male-text">M ${x.M}</b> · <b class="female-text">K ${x.F}</b></small></div>`}).join('')}</div>`;
  }

  function renderSexYearTrend(){
    const el=document.querySelector('#yearTrend');if(!el)return;const rows=filteredAcrossYears({ignoreSex:true}),years=state.data.races.slice().sort((a,b)=>a.year-b.year),data=years.map(r=>{const rr=rows.filter(x=>x.race_id===r.id&&isFinished(x));return{year:r.year,M:safeMed(rr.filter(x=>sexKey(x)==='M').map(x=>x.finish_seconds)),F:safeMed(rr.filter(x=>sexKey(x)==='F').map(x=>x.finish_seconds))}}).filter(x=>x.M||x.F);if(!data.length){el.innerHTML='<div class="empty">Ingen årsdata finns.</div>';return}
    const W=780,H=270,p={l:70,r:22,t:24,b:43},all=data.flatMap(d=>[d.M,d.F].filter(Number.isFinite)),lo=Math.floor(Math.min(...all)/1800)*1800,hi=Math.ceil(Math.max(...all)/1800)*1800,x=i=>data.length===1?(p.l+W-p.r)/2:p.l+i*(W-p.l-p.r)/(data.length-1),y=v=>p.t+(hi-v)*(H-p.t-p.b)/(hi-lo||1);let out='';for(let i=0;i<=4;i++){const v=hi-(hi-lo)*i/4,yy=y(v);out+=svg('line',{x1:p.l,y1:yy,x2:W-p.r,y2:yy,class:'gridline'})+svg('text',{x:p.l-10,y:yy+4,'text-anchor':'end'},fmtTime(v).slice(0,-3))}out+=svg('line',{x1:p.l,y1:p.t,x2:p.l,y2:H-p.b,class:'axis'})+svg('text',{x:17,y:(p.t+H-p.b)/2,'text-anchor':'middle',transform:`rotate(-90 17 ${(p.t+H-p.b)/2})`},'Median sluttid');
    [['M',COLORS.male],['F',COLORS.female]].forEach(([s,c])=>{const valid=data.map((d,i)=>({...d,i})).filter(d=>d[s]);if(!valid.length)return;out+=`<path d="${valid.map((d,j)=>`${j?'L':'M'}${x(d.i)} ${y(d[s])}`).join(' ')}" fill="none" stroke="${c}" stroke-width="3.5"/>`;valid.forEach(d=>out+=`<circle cx="${x(d.i)}" cy="${y(d[s])}" r="5" fill="${c}" stroke="#fff" stroke-width="2"><title>${d.year} · ${sexLabel(s)}: ${fmtTime(d[s])}</title></circle>`)});data.forEach((d,i)=>out+=svg('text',{x:x(i),y:H-14,'text-anchor':'middle'},String(d.year)));el.innerHTML=`<div class="chart-inline-legend">${chartLegend()}</div><svg viewBox="0 0 ${W} ${H}">${out}</svg>`;
  }

  function patchNerdCharts(){
    renderPercentiles=function(){
      const el=document.querySelector('#percentileLadder');if(!el)return;const rows=state.filtered.filter(isFinished),m=rows.filter(r=>sexKey(r)==='M').map(r=>r.finish_seconds),f=rows.filter(r=>sexKey(r)==='F').map(r=>r.finish_seconds);if(m.length<2&&f.length<2){el.innerHTML='<div class="empty">Fler sluttider krävs</div>';return}
      const levels=[[1,.01,'Topp 1 %'],[5,.05,'Topp 5 %'],[10,.10,'Topp 10 %'],[25,.25,'Topp 25 %'],[50,.50,'Median'],[75,.75,'75-percentilen']];el.innerHTML=`<div class="percentile-sex-head">${chartLegend()}</div>`+levels.map(([,q,label])=>`<div class="percentile-sex-step"><span>${label}</span><div class="percentile-dual"><div class="male"><small>Män</small><strong>${fmtTime(quantile(m,q))}</strong></div><div class="female"><small>Kvinnor</small><strong>${fmtTime(quantile(f,q))}</strong></div></div></div>`).join('');
    };
  }
  ...
  function renderGenderWorld(){
    ...
    renderSexPaceChart(document.querySelector('#genderPaceChart'),rows,true,false);renderSexPaceChart(document.querySelector('#genderRetentionChart'),rows,false,true);renderGenderHistory();renderGenderInsights(m,f);
  }
  function renderGenderHistory(){
    const el=document.querySelector('#genderHistoryChart'),all=filteredAcrossYears({ignoreSex:true}),years=state.data.races.slice().sort((a,b)=>a.year-b.year),data=years.map(r=>{const rr=all.filter(x=>x.race_id===r.id),m=rr.filter(x=>sexKey(x)==='M'&&!isDns(x)),f=rr.filter(x=>sexKey(x)==='F'&&!isDns(x));return{year:r.year,M:m.length,F:f.length,MR:pct(m.filter(isFinished).length,m.length),FR:pct(f.filter(isFinished).length,f.length)}}),W=800,H=285,p={l:52,r:54,t:24,b:44},maxN=Math.max(...data.map(d=>d.M+d.F),1),x=i=>p.l+i*(W-p.l-p.r)/(data.length-1||1),yN=v=>p.t+(maxN-v)*(H-p.t-p.b)/maxN,yP=v=>p.t+(100-v)*(H-p.t-p.b)/100;let out='';for(let i=0;i<=4;i++){const yy=p.t+(H-p.t-p.b)*i/4;out+=svg('line',{x1:p.l,y1:yy,x2:W-p.r,y2:yy,class:'gridline'})+svg('text',{x:7,y:yy+4},String(Math.round(maxN*(1-i/4))))+svg('text',{x:W-45,y:yy+4},`${100-i*25}%`)}
    data.forEach((d,i)=>{const bw=Math.max(8,(W-p.l-p.r)/(data.length*2.7)),xx=x(i)-bw;out+=`<rect x="${xx}" y="${yN(d.M)}" width="${bw}" height="${H-p.b-yN(d.M)}" fill="${COLORS.male}" opacity=".75"><title>${d.year}: ${d.M} män</title></rect><rect x="${xx+bw}" y="${yN(d.F)}" width="${bw}" height="${H-p.b-yN(d.F)}" fill="${COLORS.female}" opacity=".75"><title>${d.year}: ${d.F} kvinnor</title></rect>`+svg('text',{x:x(i),y:H-14,'text-anchor':'middle'},String(d.year))});[['MR',COLORS.male],['FR',COLORS.female]].forEach(([k,c])=>{out+=`<path d="${data.map((d,i)=>`${i?'L':'M'}${x(i)} ${yP(d[k])}`).join(' ')}" fill="none" stroke="${c}" stroke-width="3" stroke-dasharray="7 5"/>`});el.innerHTML=`<div class="chart-inline-legend">Staplar = startande · streckad linje = fullföljandegrad · ${chartLegend()}</div><svg viewBox="0 0 ${W} ${H}">${out}</svg>`;
  }
  ...
  function renderClubWorld(){
    ...
    if(!stats.length){if(profile)profile.innerHTML='<div class="empty panel">Inga klubbar eller orter i urvalet.</div>';if(search)search.value='';if(select)select.value='';if(dna)dna.innerHTML='<div class="empty">Ingen klubb/ort vald.</div>';if(history)history.innerHTML='<div class="empty">Ingen klubb/ort vald.</div>';return}
    const keys=stats.map(x=>x.key),selectedKey=keys.includes(select?.value)?select.value:'';
    if(select)select.value=selectedKey;
    if(search&&document.activeElement!==search)search.value=selectedKey?(stats.find(x=>x.key===selectedKey)?.name||''):'';
    ...
    else{if(profile)profile.innerHTML='<div class="empty panel club-profile-placeholder"><strong>Sök fram en klubb eller ort</strong><span>Ingen profil är förvald.</span></div>';if(dna)dna.innerHTML='<div class="empty">Sök och välj en klubb/ort för att visa klubbens profil.</div>';if(history)history.innerHTML='<div class="empty">Sök och välj en klubb/ort för att visa historiken.</div>'}
  }
