...
if(typeof window!=='undefined'&&typeof document!=='undefined')(() => {
  ...
  function renderGenderHistory(){
    ...
    const all=filteredAcrossYears({ignoreSex:true}),years=familyRaces().slice().sort((a,b)=>a.year-b.year),data=years.map(r=>{const rr=all.filter(x=>x.race_id===r.id),groups={};['M','F'].forEach(s=>{const starters=rr.filter(x=>sexKey(x)===s&&!isDns(x)),finishers=starters.filter(isFinished),dnf=starters.filter(isDnf);groups[s]={starters:starters.length,finishers:finishers.length,dnf:dnf.length,rate:starters.length?pct(finishers.length,starters.length):null}});return{year:r.year,...groups}}),W=840,H=330,p={l:66,r:72,t:48,b:48},plotHeight=H-p.t-p.b,maxN=Math.max(1,...data.flatMap(d=>selected.map(s=>d[s].starters))),roughStep=maxN/4,power=10**Math.floor(Math.log10(roughStep)),unit=roughStep/power,countStep=(unit<=1?1:unit<=2?2:unit<=5?5:10)*power,countMax=Math.ceil(maxN/countStep)*countStep,x=i=>p.l+i*(W-p.l-p.r)/(data.length-1||1),yN=v=>H-p.b-v/countMax*plotHeight,yP=v=>p.t+(100-v)*plotHeight/100;let axes='',bars='',lines='';
  ...
  function renderClubHistory(stats){
    ...
    const family=familyResults(),series=selected.map((club,ci)=>{const rows=family.filter(r=>advanced.clubKeyByResult.get(r.id)===club.key);return{name:club.name,color:CLASS_COLORS[ci%CLASS_COLORS.length],pts:years.map(r=>{const rr=rows.filter(x=>x.race_id===r.id),starters=rr.filter(x=>!isDns(x)),finishers=rr.filter(isFinished);return{year:r.year,starters:starters.length,finishers:finishers.length,med:safeMed(finishers.map(x=>x.finish_seconds))}})}}),meds=series.flatMap(s=>s.pts.map(d=>d.med).filter(Number.isFinite));
