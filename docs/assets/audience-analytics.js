...
if(typeof module!=='undefined'&&module.exports)module.exports={audienceRaceFamily,selectAudienceRace,aggregateSpeedKmh,speedIndex};
...
if(typeof window!=='undefined'&&typeof document!=='undefined')(() => {
  ...
  function renderClassHistory(){
    ...
    const data=classes.map((k,ki)=>({k,color:CLASS_COLORS[ki%CLASS_COLORS.length],pts:years.map(r=>{const rr=all.filter(x=>x.race_id===r.id&&normClass(x.age_class)===k),st=rr.filter(x=>!isDns(x)),fin=rr.filter(isFinished),dnf=st.filter(isDnf);return{year:r.year,med:safeMed(fin.map(x=>x.finish_seconds)),starters:st.length,dnf:dnf.length}})})),allT=data.flatMap(s=>s.pts.map(x=>x.med).filter(Number.isFinite));if(!allT.length){el.innerHTML='<div class="empty">Historik saknas för valda klasser.</div>';return}
    const W=900,H=410,p={l:72,r:28,t:26,b:34},lineBottom=238,barTop=270,barBottom=370,lo=Math.floor(Math.min(...allT)/1800)*1800,hi=Math.ceil(Math.max(...allT)/1800)*1800,maxN=Math.max(1,...data.flatMap(s=>s.pts.map(d=>d.starters))),x=i=>p.l+i*(W-p.l-p.r)/(years.length-1||1),yT=v=>p.t+(hi-v)*(lineBottom-p.t)/(hi-lo||1),yN=v=>barBottom-v*(barBottom-barTop)/maxN,step=(W-p.l-p.r)/(years.length-1||1),groupWidth=Math.min(62,step*.72),pairWidth=Math.max(2,groupWidth/(classes.length*2+1));let out='';
    for(let i=0;i<=4;i++){const v=hi-(hi-lo)*i/4,yy=yT(v);out+=svg('line',{x1:p.l,y1:yy,x2:W-p.r,y2:yy,class:'gridline'})+svg('text',{x:p.l-9,y:yy+4,'text-anchor':'end'},fmtTime(v).slice(0,-3))}
    out+=svg('line',{x1:p.l,y1:barBottom,x2:W-p.r,y2:barBottom,class:'axis'})+svg('text',{x:p.l-9,y:barTop+4,'text-anchor':'end',class:'class-history-count-label'},String(maxN))+svg('text',{x:p.l-9,y:barBottom+4,'text-anchor':'end',class:'class-history-count-label'},'0');
    data.forEach((s,si)=>{const valid=s.pts.map((d,i)=>({...d,i})).filter(d=>Number.isFinite(d.med));out+=`<path class="class-history-line" d="${valid.map((d,j)=>`${j?'L':'M'}${x(d.i)} ${yT(d.med)}`).join(' ')}" fill="none" stroke="${s.color}" stroke-width="3"/>`;valid.forEach(d=>{const tip=`${s.k} ${d.year} · median ${fmtTime(d.med)}`,px=x(d.i),py=yT(d.med);out+=`<circle class="interactive-chart-point class-history-point" cx="${px}" cy="${py}" r="4.8" fill="${s.color}" tabindex="0" role="img" aria-label="${esc(tip)}" data-chart-tip="${esc(tip)}" data-chart-x="${(px/W*100).toFixed(2)}" data-chart-y="${(py/H*100).toFixed(2)}"><title>${esc(tip)}</title></circle>`});s.pts.forEach((d,i)=>{const base=x(i)-groupWidth/2+si*pairWidth*2,startX=base,dnfX=base+pairWidth,startY=yN(d.starters),dnfY=yN(d.dnf),startTip=`${s.k} ${d.year} · ${d.starters} startande`,dnfTip=`${s.k} ${d.year} · ${d.dnf} DNF`;out+=`<rect class="class-history-bar starters" x="${startX}" y="${startY}" width="${pairWidth-.8}" height="${barBottom-startY}" fill="${s.color}" tabindex="0" role="img" aria-label="${esc(startTip)}" data-chart-tip="${esc(startTip)}" data-chart-x="${(startX/W*100).toFixed(2)}" data-chart-y="${(startY/H*100).toFixed(2)}"><title>${esc(startTip)}</title></rect><rect class="class-history-bar dnf" x="${dnfX}" y="${dnfY}" width="${pairWidth-.8}" height="${barBottom-dnfY}" fill="${s.color}" tabindex="0" role="img" aria-label="${esc(dnfTip)}" data-chart-tip="${esc(dnfTip)}" data-chart-x="${(dnfX/W*100).toFixed(2)}" data-chart-y="${(dnfY/H*100).toFixed(2)}"><title>${esc(dnfTip)}</title></rect>`})});
    years.forEach((r,i)=>out+=svg('text',{x:x(i),y:H-9,'text-anchor':'middle'},String(r.year)));
    el.innerHTML=`<div class="class-chart-legend">${data.map(s=>`<span><i style="background:${s.color}"></i>${esc(s.k)}</span>`).join('')}</div><div class="class-history-key"><span><i class="median"></i>Median</span><span><i class="starters"></i>Startande</span><span><i class="dnf"></i>DNF</span></div><div class="interactive-chart-tooltip" role="status" hidden></div><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Klasshistorik med medianlinjer och staplar för startande och DNF">${out}</svg>`;wireChartTooltips(el);
  }
  ...
  function renderClubCompare(stats){
    ...
    const W=800,H=300,p={l:58,r:24,t:22,b:74},paceMode=currentSpeedUnit()==='pace',series=selected.map((c,i)=>({name:c.name,color:CLASS_COLORS[i%CLASS_COLORS.length],speeds:cps.map(cp=>safeMed(segmentSpeeds(c.rows,cp.sequence_no)))})).map(s=>({...s,pts:s.speeds.map(speed=>Number.isFinite(speed)?speedDisplayValue(speed):null)})),all=series.flatMap(s=>s.pts.filter(Number.isFinite));if(!all.length){el.innerHTML='<div class="empty">Mellantider saknas för valda klubbar.</div>';return}
    const lo=Math.min(...all),hi=Math.max(...all),x=i=>p.l+i*(W-p.l-p.r)/(cps.length-1||1),y=v=>paceMode?p.t+(v-lo)*(H-p.t-p.b)/(hi-lo||1):p.t+(hi-v)*(H-p.t-p.b)/(hi-lo||1);let out='';for(let i=0;i<=4;i++){const v=paceMode?lo+(hi-lo)*i/4:hi-(hi-lo)*i/4,yy=y(v);out+=svg('line',{x1:p.l,y1:yy,x2:W-p.r,y2:yy,class:'gridline'})+svg('text',{x:p.l-8,y:yy+4,'text-anchor':'end'},formatSpeedAxis(v))}series.forEach(s=>{const valid=s.pts.map((v,i)=>({v,i})).filter(point=>Number.isFinite(point.v));out+=`<path d="${valid.map((d,j)=>`${j?'L':'M'}${x(d.i)} ${y(d.v)}`).join(' ')}" fill="none" stroke="${s.color}" stroke-width="3"/>`});pairs.forEach((pair,i)=>out+=svg('text',{x:x(i),y:H-31,'text-anchor':'end',transform:`rotate(-30 ${x(i)} ${H-31})`},pair.label));el.innerHTML=`<div class="class-chart-legend">${series.map(s=>`<span><i style="background:${s.color}"></i>${esc(s.name)}</span>`).join('')}<span>${esc(window.SpeedUnits.unitLabel())}</span></div><svg viewBox="0 0 ${W} ${H}">${out}</svg>`
  }
