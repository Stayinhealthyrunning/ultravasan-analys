...
if(typeof module!=='undefined'&&module.exports)module.exports={audienceRaceFamily,selectAudienceRace,aggregateSpeedKmh,speedIndex,relativeToplistWidth,visibleCountBarHeight,fixedFinishBins,finishBinTime:audienceFinishBinTime,selectedClubProfiles,mergedClubFinishers};
...
if(typeof window!=='undefined'&&typeof document!=='undefined')(() => {
  ...
  function patchOverviewCharts(){
    renderHistogram=function(){
      ...
      const series=visibleSexes('histogram'),W=Math.max(680,bins.length*34+61),H=280,p={l:45,r:16,t:18,b:44},cw=(W-p.l-p.r)/bins.length,ymax=Math.max(...counts.map(c=>series.reduce((sum,s)=>sum+c[s],0)),1);let out='';
      ...
      counts.forEach((c,i)=>{const x=p.l+i*cw+2,bin=bins[i];let base=H-p.b;series.forEach(k=>{const color=k==='M'?COLORS.male:COLORS.female;if(!c[k])return;const h=(H-p.t-p.b)*c[k]/ymax;base-=h;const tip=`${sexLabel(k)}: ${c[k]} · ${audienceFinishBinTime(bin.from)}–${audienceFinishBinTime(bin.to)}`;out+=`<rect x="${x}" y="${base}" width="${Math.max(2,cw-4)}" height="${h}" rx="2" fill="${color}" class="sex-bar" tabindex="0" role="img" aria-label="${esc(tip)}"><title>${esc(tip)}</title></rect>`});if(i%Math.max(1,Math.ceil(bins.length/7))===0||i===bins.length-1)out+=svg('text',{x,y:H-18},audienceFinishBinTime(bin.from))});
      out+=svg('line',{x1:p.l,y1:H-p.b,x2:W-p.r,y2:H-p.b,class:'axis'});el.innerHTML=`<svg viewBox="0 0 ${W} ${H}" style="min-width:${W}px">${out}</svg>`;
