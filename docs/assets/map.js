...
async function ensureRaceData(){const quick=readSessionData();if(quick){setLoading('Läser de valda löparna…');return quick}if(window.UltravasanDataLoader){setLoading('Läser bara de loppår som kartan behöver…');const dataset=await window.UltravasanDataLoader.loadInitial(),params=new URLSearchParams(location.search),requestedIds=(params.get('runners')||'').split(',').map(Number).filter(Boolean);if(requestedIds.length)await window.UltravasanDataLoader.ensureResults(requestedIds);else{const requested=Number(params.get('year')),requestedFamily=['uv90','uv45'].includes(params.get('race'))?params.get('race'):'uv90',familyRaces=dataset.races.filter(r=>mapRaceFamily(r)===requestedFamily).sort((a,b)=>b.year-a.year),race=familyRaces.find(r=>r.id===requested||r.year===requested)||familyRaces[0];if(race)await window.UltravasanDataLoader.ensureRace(race.id)}return window.UltravasanDataLoader.data}if(window.ULTRAVASAN_DATA)return window.ULTRAVASAN_DATA;setLoading('Läser historikdatabasen för den delade kartlänken…');await loadScript('data/ultravasan-data.js');if(!window.ULTRAVASAN_DATA)throw new Error('Historikdatabasen laddades inte.');return window.ULTRAVASAN_DATA}
...
function buildDuelElevation(){
  const root=$('#duelElevation'),svgEl=$('#duelElevationChart'),route=activeReferenceRoute(app.models,app.usedRoutes,app.registry),profile=route?.elevation_profile||[];
  if(!root||!svgEl)return;
  if(profile.length<2){root.classList.add('unavailable');svgEl.innerHTML='<text x="600" y="52" text-anchor="middle" class="duel-elevation-empty">Höjdprofil saknas för den här banan.</text>';return}
  root.classList.remove('unavailable');$('#duelElevationRoute').textContent=route.style?.label||`${Number(route.official_distance_km).toFixed(1)} km`;
  const v=DUEL_ELEVATION_VIEW,total=Number(route.official_distance_km)||Number(profile.at(-1)[0])||1,elevations=profile.map(point=>Number(point[1])).filter(Number.isFinite),min=Math.min(...elevations),max=Math.max(...elevations),range=max-min||1,x=d=>v.left+clamp(Number(d)||0,0,total)/total*(v.width-v.left-v.right),y=e=>v.top+(max-Number(e))/(range)*(v.height-v.top-v.bottom),line=profile.map((point,index)=>`${index?'L':'M'}${x(point[0]).toFixed(1)} ${y(point[1]).toFixed(1)}`).join(' '),base=v.height-v.bottom;
  let html=`<defs><linearGradient id="duelElevationFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#dbe75a" stop-opacity=".52"/><stop offset="1" stop-color="#176d53" stop-opacity=".08"/></linearGradient></defs><line class="duel-elevation-axis" x1="${v.left}" x2="${v.width-v.right}" y1="${base}" y2="${base}"/><path class="duel-elevation-area" d="${line} L${x(total)} ${base} L${x(0)} ${base} Z"/><path class="duel-elevation-line" d="${line}"/>`;
  for(const cp of route.checkpoints||[]){const px=x(cp.distance_km);html+=`<g class="duel-elevation-checkpoint"><line x1="${px}" x2="${px}" y1="${v.top}" y2="${base}"/><circle cx="${px}" cy="${y(elevationAtDistance(route,cp.distance_km))}" r="2.8"/><title>${esc(cp.name||cp.short||cp.key)} · ${Number(cp.distance_km).toFixed(1)} km</title></g>`}
  for(const model of app.models)html+=`<circle id="duelElevationRunner${model.result.id}" class="duel-elevation-runner" cx="${x(0)}" cy="${y(elevationAtDistance(route,0))}" r="6" fill="${model.color}" aria-label="${esc(model.result.name_as_published)} vid start"><title>${esc(model.result.name_as_published)}</title></circle>`;
  svgEl.innerHTML=html;app.duelElevation={route,profile,min,max,total,x,y};
}
function updateDuelElevation(states){
...
function frame(now){if(!app.playing)return;const dt=Math.min(.1,(now-app.lastFrame)/1000);app.lastFrame=now;const next=Math.min(app.maxTime,app.time+dt*duelPlaybackRate(app.maxTime,app.speed));checkEvents(app.time,next);app.prevTime=app.time;app.time=next;update();if(app.time>=app.maxTime){app.playing=false;$('#playBtn').textContent='▶';setEvent('Alla valda löpare har nått sin sista registrerade position. Musiken fortsätter tills du stänger av den.')}else requestAnimationFrame(frame)}
