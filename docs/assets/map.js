...
async function ensureRaceData(){const quick=readSessionData();if(quick){setLoading('Läser de valda löparna…');return quick}if(window.UltravasanDataLoader){setLoading('Läser löparindex och nödvändiga loppdata…');const data=await window.UltravasanDataLoader.loadCore();const params=new URLSearchParams(location.search),requestedIds=(params.get('runners')||'').split(',').map(Number).filter(Boolean);if(requestedIds.length)await window.UltravasanDataLoader.ensureResults(requestedIds);else{const requested=Number(params.get('year')),requestedFamily=['uv90','uv45'].includes(params.get('race'))?params.get('race'):'uv90',familyRaces=data.races.filter(r=>window.RaceContracts.familyForRace(r)===requestedFamily),race=familyRaces.find(r=>r.id===requested||r.year===requested)||familyRaces.slice().sort((a,b)=>b.year-a.year)[0];if(race)await window.UltravasanDataLoader.ensureEdition(race.id)}return data}if(window.ULTRAVASAN_DATA)return window.ULTRAVASAN_DATA;setLoading('Läser historikdatabasen för den delade kartlänken…');await loadScript('data/ultravasan-data.js');if(!window.ULTRAVASAN_DATA)throw new Error('Historikdatabasen laddades inte.');return window.ULTRAVASAN_DATA}
