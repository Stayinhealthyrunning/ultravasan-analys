...
function sprintClassOptions(rows,sex=''){
...
function finishSprintRanking(results,{getSplits=()=>[],isFinished=row=>String(row?.status||'').toUpperCase()==='FINISHED',classKey='',womenClass='',menClass='',raceDistanceKm=null,maxSpeedKmh=30}={}){
  ...
  const eligible=(results||[]).map(row=>{
    ...
    const sprint=finish-warned,warningDistance=optionalNumber(warning.distance_km),finishDistance=optionalNumber(raceDistanceKm);
    // Avståndskontrollen används bara när båda avstånden faktiskt finns.
    // NULL får aldrig tolkas som 0 km; UV90 2026 har verifierad Förvarning-tid
    // men saknar explicit distance_km för just den servicepunkten.
    if(warningDistance!==null&&finishDistance!==null&&finishDistance>warningDistance){
...
const SPRINT_WINNER_METHOD_HELP='Spurtvinnaren använder endast fullföljare i valt loppår som har en faktiskt registrerad, ej estimerad passage vid Mora Förvarning. Spurttiden är officiell sluttid minus tiden vid Mora Förvarning. Kartans referenspunkt används aldrig som ersättning för en saknad mellantid. En avståndsbaserad plausibilitetskontroll görs bara när Förvarningens och målets avstånd båda finns explicit i resultatdatan; saknat avstånd tolkas aldrig som 0 km. Startläget är aktuellt loppår och alla klasser. Kvinnor och män har separata klassfilter och rangordnas oberoende; lika spurttid ger delad placering och delad femteplats visas i sin helhet.';
...
function initNerdLab(){
  ...
  n$('#sprintClassWomen')?.addEventListener('change',renderSprintWinners);
  n$('#sprintClassMen')?.addEventListener('change',renderSprintWinners);
...
function populateSprintClassFilters(rows){
  const populate=(select,sex,allLabel)=>{
    if(!select)return'';
    const sameRace=select.dataset.race===String(state.raceId),old=sameRace?select.value:'',options=sprintClassOptions(rows,sex);
    select.innerHTML=`<option value="">${allLabel}</option>`+options.map(option=>`<option value="${nEsc(option)}">${nEsc(option)}</option>`).join('');
...
function renderSprintWinners(){
  const women=n$('#sprintWomen'),men=n$('#sprintMen'),womenCoverage=n$('#sprintWomenCoverage'),menCoverage=n$('#sprintMenCoverage');if(!women||!men)return;
  const race=activeRace();
  if(!race){
    women.innerHTML=men.innerHTML='<div class="empty compact-empty">Loppår saknas.</div>';
    if(womenCoverage)womenCoverage.textContent='';if(menCoverage)menCoverage.textContent='';return
  }
  const raceRows=state.data.results.filter(row=>String(row.race_id)===String(race.id));
  const {womenClass,menClass}=populateSprintClassFilters(raceRows);
  const model=finishSprintRanking(raceRows,{getSplits:nAuxSplitsForResult,isFinished:nIsFinished,womenClass,menClass,raceDistanceKm:race.distance_km});
  ...
  if(womenCoverage)womenCoverage.textContent=model.women.length?`${race.year} · ${model.women.length.toLocaleString('sv-SE')} giltiga spurttider`:`${race.year} · ingen verifierad spurttid`;
  if(menCoverage)menCoverage.textContent=model.men.length?`${race.year} · ${model.men.length.toLocaleString('sv-SE')} giltiga spurttider`:`${race.year} · ingen verifierad spurttid`;
