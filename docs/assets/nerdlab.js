...
const SPRINT_WINNER_METHOD_HELP='Spurtvinnaren använder endast fullföljare i valt loppår som har en faktiskt registrerad, ej estimerad passage vid den sista officiella mellantidskontrollen efter Eldris och före mål. Källans kontrollnamn får variera mellan år; den väljs efter sin position i kontrollföljden, inte efter etiketten. Spurttiden är officiell sluttid minus tiden vid denna kontroll. Kartans referenspunkt används aldrig som ersättning för en saknad mellantid. Orimligt hög fart filtreras bort när återstående distans är verifierbar. Startläget är alla klasser i aktuellt loppår. Kvinnor och män har separata klassfilter som bara påverkar respektive topplista. Lika spurttid ger delad placering och delad femteplats visas i sin helhet.';
...
const CLASS_HISTORY_METHOD_HELP='Klasshistorik är en beskrivande årsserie med två separata lägen för att undvika dubbla axlar och överlastning. Mediantid visar median sluttid för fullföljande per vald klass och linjen bryts vid kalenderluckor utan lopp. Deltagande visar alla faktiska startande i en stapel per klass och år, där den mörka delen är DNF; DNS räknas inte som startande. DNF-andelen beräknas som DNF/startande och visas tillsammans med antalet i tooltip. Bansträckningen har förändrats över historiken, så skillnader mellan år är beskrivande och inte ett bevis på ren prestationsutveckling; tekniska jämförbarhetsgränser visas inte som egna linjer i diagrammet.';
...
function renderSprintWinners(){
  ...
  const hasSprintControl=raceRows.some(row=>Boolean(nMoraWarningSplit(nAuxSplitsForResult(row.id))));
  ...
  const render=(list,label)=>{
    if(!hasSprintControl)return `<div class="empty compact-empty">Ingen officiell mellantidskontroll efter Eldris och före mål finns i resultatunderlaget för ${race.year}. Spurtvinnare kan därför inte beräknas för detta loppår.</div>`;
    const selected=top(list);
    return selected.length?selected.map(rowHtml).join(''):`<div class="empty compact-empty">Ingen ${label} har en giltig registrerad passage vid den sista kontrollen före mål i detta urval.</div>`;
  };
  women.innerHTML=render(womenModel.women,'kvinna');men.innerHTML=render(menModel.men,'man');
  if(coverage)coverage.textContent=hasSprintControl
    ?`${race.year} · ${womenModel.rows.length.toLocaleString('sv-SE')} giltiga kvinnliga spurttider · ${menModel.rows.length.toLocaleString('sv-SE')} giltiga manliga spurttider`
    :`${race.year} · sista officiella mellantidskontroll efter Eldris saknas i resultatunderlaget`;
