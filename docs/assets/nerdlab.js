...
function renderCoursePaceView(model,selected){
  ...
  const rows=model.segments.map(segment=>{
    ...
    const outer=segment.field?.outer_spread_sufficient?`${nPace(segment.field.q10_pace_seconds_per_km)}–${nPace(segment.field.q90_pace_seconds_per_km)}`:'n<20';
    return `<button type="button" class="course-pace-row ${segment.key===selected?.key?'selected':''}" data-course-segment="${nEsc(segment.key)}" ${width?'':'disabled'}><span>${nEsc(segment.from_name)} → ${nEsc(segment.to_name)}</span><i><b style="width:${width}%"></b></i><strong>${Number.isFinite(Number(value))?nPace(value):'–'}</strong><small>Q25–Q75 ${nEsc(spread)} · Q10–Q90 ${nEsc(outer)}</small></button>`;
  }).join('');
  el.innerHTML=`<div class="course-view-head"><span>FARTFÖRDELNING</span><strong>Median · Q25–Q75 · Q10–Q90 vid n≥20</strong></div><div class="course-pace-list">${rows}</div>`;
  bindCourseSegmentClicks(el);
}
...
function renderCourseIntelligence(existingModel=null){
  ...
  rowsEl.innerHTML=model.segments.map(segment=>{
    ...
    const outerRange=field.outer_spread_sufficient&&field.q10_pace_seconds_per_km!=null&&field.q90_pace_seconds_per_km!=null?`${nPace(field.q10_pace_seconds_per_km)}–${nPace(field.q90_pace_seconds_per_km)}`:'–';
