...
const HISTORY_FINGERPRINT_METHOD_HELP='Årets fingeravtryck sätter index 100 till historisk normalnivå. Mediantidsindex, fartnivå och DNF-belastning jämför endast RaceEditions med samma uttryckligt verifierade bansträckningsserie; bansträckningskontraktet definierar kontroller och delsträckor men avgör inte ensamt helbanans jämförbarhet. Varje jämförbart loppår sammanfattas separat och normalnivån är medianen av loppårsmedianerna. Minst två andra jämförbara loppår krävs. Faktiska referensår och exkluderade år visas. Kvinnorepresentation och fältstorlek får använda andra bansträckningar inom samma loppfamilj; kvinnorepresentation döljs när könsfilter är aktivt. Index beskriver observerad skillnad, inte orsak.';
const CLASS_HISTORY_METHOD_HELP='Klasshistorik och Klassutveckling visar deltagande över alla importerade år, men fart- och sluttidstrender får endast bindas samman inom samma uttryckligt verifierade bansträckningsserie. Bansträckningskontraktet beskriver kontroller och delsträckor och skapar inte ensamt en historisk brytpunkt. När helbanans jämförbarhet saknas visas separata punkter och ingen fartinterpolation; en verklig gruppgräns märks som ny jämförbarhetsserie. Deltagarantal kan jämföras över dessa gränser. Kalenderluckor utan lopp märks uttryckligen. DNS räknas inte som startande; medianfart och sluttid bygger på fullföljande med giltig sluttid.';
...
function renderFingerprint(){
  ...
  }).join('')+`<p class="microcopy">${race.year}: prestationsmått använder ${model.performance_reference_years.length} verifierade bansträckningsreferensår (${model.performance_reference_years.join(', ')||'inga'}). ${model.performance_exclusions.length?'Övriga loppår utesluts eftersom helbanans jämförbarhet inte är verifierad; byte av bansträckning eller kontrollstruktur avgör inte ensamt jämförbarheten.':''} Index 100 = medianen av loppårsnormalerna.</p>`;
}
