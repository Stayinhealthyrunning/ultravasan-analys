...
(function(root,factory){
  ...
  function aggregateClassHistory({races=[],results=[],getClass=result=>result.age_class,getGender=result=>result.sex,isStarter=()=>true,isFinished=result=>Number(result.finish_seconds)>0}={}){
    ...
    for(const group of groups.values()){
      ...
      points.push({raceId:group.raceId,year:group.year,className:group.className,gender,participantCount:group.participantCount,finisherCount:group.finisherCount,medianSpeedKmh:median(group.speeds),medianFinishTimeSeconds:median(group.times),validResultCount,previousYear:null,participantDelta:null,speedDelta:null});
    }
    ...
    for(const classPoints of Object.values(pointsByClass))for(let i=1;i<classPoints.length;i++){const previous=classPoints[i-1],current=classPoints[i];current.previousYear=previous.year;current.participantDelta=current.participantCount-previous.participantCount;current.speedDelta=current.medianSpeedKmh-previous.medianSpeedKmh}
  ...
  function pointTooltip(point,raceName,participantLabel){
    ...
    if(Number.isFinite(point.speedDelta))deltas.push(`${point.speedDelta>0?'+':point.speedDelta<0?'−':''}${formatDecimal(Math.abs(point.speedDelta))} km/h`);
    return`${raceName} · ${point.year} · ${point.className} · ${point.participantCount} ${participantLabel} · ${point.finisherCount} fullföljande · median ${formatDecimal(point.medianSpeedKmh)} km/h · ${formatTime(point.medianFinishTimeSeconds)} · ${point.validResultCount} giltiga resultat${deltas.length?` · sedan ${point.previousYear}: ${deltas.join(' · ')}`:''}`;
  }
  ...
  class ClassEvolutionController{
    ...
    _drawAxes(){
      const p=this._padding(),{min,max}=this._speedDomain();this.axisLayer.replaceChildren();
      for(let i=0;i<=5;i++){const value=min+(max-min)*i/5,y=this._y(value),line=svgElement('line',{x1:p.l,y1:y,x2:this.viewWidth-p.r,y2:y,class:'gridline'}),text=svgElement('text',{x:p.l-12,y:y+4,'text-anchor':'end'});text.textContent=formatDecimal(value);this.axisLayer.append(line,text)}
    ...
    _speedDomain(){const lo=Number(this.model?.minSpeed),hi=Number(this.model?.maxSpeed);if(!Number.isFinite(lo)||!Number.isFinite(hi))return{min:0,max:1};const margin=Math.max(.18,(hi-lo)*.09);return{min:Math.max(0,lo-margin),max:hi+margin}}
    ...
    _y(speed){const p=this._padding(),domain=this._speedDomain();return 560-p.b-(Number(speed)-domain.min)*(560-p.t-p.b)/(domain.max-domain.min||1)}
  ...
  return{aggregateClassHistory,bubbleRadius,animationTiming,classColor,easeInOutCubic,transitionBubble,pointTooltip,ClassEvolutionController,createController};
});
