...
(function(root,factory){
  ...
  function aggregateClassHistory({races=[],results=[],getClass=result=>result.age_class,getGender=result=>result.sex,isStarter=()=>true,isFinished=result=>Number(result.finish_seconds)>0,comparisonKeyForRace=()=>null}={}){
    ...
    for(let i=1;i<years.length;i++){
      const fromYear=years[i-1],toYear=years[i],fromKey=raceComparisonByYear.get(fromYear)||null,toKey=raceComparisonByYear.get(toYear)||null;
      if(!fromKey||!toKey||fromKey!==toKey)comparisonBreaks.push({fromYear,toYear,fromKey,toKey});
    }
    return{years,races:sortedRaces,points,pointsByClass,classes,maxParticipantCount:Math.max(1,...points.map(point=>point.participantCount)),minSpeed:speeds.length?Math.min(...speeds):null,maxSpeed:speeds.length?Math.max(...speeds):null,participantLabel:'startande',comparisonBreaks,raceComparisonByYear:Object.fromEntries(raceComparisonByYear)};
  }
  ...
  function pointTooltip(point,raceName,participantLabel){
    ...
    if(point.comparisonBreak&&point.previousYear)deltas.push('ny banjämförbarhetsserie – farttrend bryts');
  ...
  class ClassEvolutionController{
    ...
    _updateSummary(index){const year=this.model.years[index],points=this.model.points.filter(point=>point.year===year),selected=points.filter(point=>this.selected.includes(point.className)),boundary=(this.model.comparisonBreaks||[]).find(item=>Number(item.toYear)===Number(year)),suffix=boundary?' · ny banversion: farttrend bryts':'';if(!points.length)this._setStatus(`Inga giltiga klassresultat finns för ${year}.`);else if(this.selected.length&&!selected.length)this._setStatus(`${year}: valda klasser saknar giltiga resultat. ${points.length} andra klasser visas${suffix}.`);else this._setStatus(`${year}: ${points.length} klasser · ${points.reduce((sum,point)=>sum+point.participantCount,0).toLocaleString('sv-SE')} startande${selected.length?` · ${selected.length} valda klasser markerade`:''}${suffix}.`)}
