#!/usr/bin/env python3
"""Conservative, atomic importer for official Ultravasan results.
...
DEFAULT_PUBLICATION_MONTH = 8
DEFAULT_PUBLICATION_DAY = 22
# The 2026 race was explicitly approved for the earlier August 16 launch. Future
# years use the normal, event-date-adjacent August 22 start without a dead end.
PUBLICATION_DATE_OVERRIDES = {2026: date(2026, 8, 16)}
APPLY_CONFIRMATION = "APPLY-AUTOMATIC-OFFICIAL-RESULTS"
...
def schedule_state(today: date, year: int = 2026) -> str:
    if today < publication_date(year):
...
def collect_list_entries(race: dict[str, Any], raw: Path, delay: float, force: bool = False, max_pages: int = 250) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    fetcher = mika_import.Fetcher(delay, browser_fallback=False, force=force)
...
def protected_history_digest(conn: sqlite3.Connection, target_year: int) -> str:
    """Hash historic meaning, never volatile import/catalogue timestamps.

    ``init_db`` deliberately refreshes ``races.updated_at`` while synchronising
    catalogue metadata. Hashing ``SELECT *`` therefore made a clean future-race
    import look like a historic data mutation. These explicit columns cover all
    historic race, result and split values consumed by the application.
    """
    payload = {
        "races": dump_rows(conn, "SELECT id,race_key,name,year,race_date,distance_km,event_code,result_year_path,official_url,course_version,notes FROM races WHERE year<? ORDER BY id", (target_year,)),
        "checkpoints": dump_rows(conn, "SELECT cp.id,cp.race_id,cp.checkpoint_key,cp.name,cp.sequence_no,cp.distance_km,cp.elevation_m FROM checkpoints cp JOIN races r ON r.id=cp.race_id WHERE r.year<? ORDER BY cp.id", (target_year,)),
        "results": dump_rows(conn, "SELECT res.id,res.race_id,res.athlete_id,res.source_id,res.source_result_id,res.source_url,res.bib,res.name_as_published,res.sex,res.age,res.birth_year,res.age_class,res.nationality,res.club,res.city,res.county,res.start_group,res.status,res.finish_seconds,res.gun_seconds,res.net_seconds,res.overall_place,res.gender_place,res.class_place,res.pace_seconds_per_km,res.raw_json FROM results res JOIN races r ON r.id=res.race_id WHERE r.year<? ORDER BY res.id", (target_year,)),
        "splits": dump_rows(conn, "SELECT s.id,s.result_id,s.checkpoint_id,s.elapsed_seconds,s.segment_seconds,s.place_overall,s.place_gender,s.place_class,s.pace_seconds_per_km,s.reported_pace_seconds_per_km,s.speed_kmh,s.time_of_day,s.diff_seconds,s.status,s.is_estimated,s.raw_json FROM splits s JOIN results res ON res.id=s.result_id JOIN races r ON r.id=res.race_id WHERE r.year<? ORDER BY s.id", (target_year,)),
    }
...
def command_window(args: argparse.Namespace) -> dict[str, Any]:
    ...
    state = schedule_state(today, year)
    report = {"date_utc": today.isoformat(), "target_year": year, "state": state, "active": state == "active"}
...
def command_availability(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        report = {
            ...
            "blockers": [f"official {year} events are not published yet: {exc}"],
        }
        args.report.parent.mkdir(parents=True, exist_ok=True)
        args.report.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        write_github_output(args.github_output, {"ready": False, "uv90_results": 0, "uv45_results": 0})
        return report
    generated = configured_targets(base, selected, year)
...
def command_apply(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report = {"generated_at": utc_now(), "mode": "apply", "target_year": int(getattr(args, "year", 2026)), "changed": True, "decision": "APPLIED", "export": export, "target_digest": digest}
    args.report.parent.mkdir(parents=True, exist_ok=True)
    args.report.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def command_resume_full_dry_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    first_reports = {}
    for family, meta in targets.items():
        path = args.report.parent / f"{meta['race_key']}-pass1.json"
        if not path.exists() and family == "uv45":
            path = args.report.parent / "uv45-diagnosis.json"
        first_reports[family] = json.loads(path.read_text(encoding="utf-8"))
    with uvtool.connect(args.production_db) as production:
        before_digest = target_semantic_digest(production, race_keys)
    with uvtool.connect(args.work_db) as work:
        history_before = protected_history_digest(work, year)
    ...
    report = {"generated_at": utc_now(), "mode": "resume-full-dry-run", "target_year": year,
              ...
              "second_pass_target_digest": second_digest, "work_db_sha256": sha256_file(args.work_db),
              "generated_config_sha256": sha256_file(args.generated_config), "protected_history_digest": history_before,
              "qualities": qualities, "second_pass_qualities": second_qualities,
              "first_counts": first_counts, "second_counts": second_counts, "export": export, "blockers": blockers}
    args.report.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    if blockers:
        raise RuntimeError("Resumed full dry-run blocked: " + "; ".join(blockers))
    return report
...
def build_parser() -> argparse.ArgumentParser:
    ...
    window.add_argument("--year", type=int, default=2026)
    ...
    availability.add_argument("--year", type=int, default=2026)
    ...
    dry.add_argument("--year", type=int, default=2026)
    ...
    apply_parser.add_argument("--year", type=int, default=2026)
    resume = sub.add_parser("resume-full-dry-run")
    resume.add_argument("--year", type=int, required=True)
    resume.add_argument("--production-db", type=Path, default=uvtool.DEFAULT_DB)
    resume.add_argument("--work-db", type=Path, required=True)
    resume.add_argument("--generated-config", type=Path, required=True)
    resume.add_argument("--availability-report", type=Path, required=True)
    resume.add_argument("--report", type=Path, required=True)
    resume.add_argument("--export-dir", type=Path, required=True)
    add_network_options(resume)
...
def main() -> None:
    ...
    commands = {
        ...
        "resume-full-dry-run": command_resume_full_dry_run,
