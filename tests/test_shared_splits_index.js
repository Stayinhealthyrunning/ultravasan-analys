...
assert.ok(nerdSource.includes('COURSE_PLAN_METHOD_HELP')&&nerdSource.includes('bara historiska fullföljare från exakt samma bansträckning')&&nerdSource.includes('ingen resttid fördelas genom gissning'),'U6:s loppplan ska ha en utförlig metodförklaring och explicit anti-gissningsregel');
...
assert.ok(historyIntelligenceSource.includes("scope:'same-route-family-race-medians'")&&historyIntelligenceSource.includes('performanceYears.length>=minReferenceYears')&&historyIntelligenceSource.includes('performanceRouteKey'),'fingeravtryckets prestationsreferens ska byggas av tidigare loppårsnormaler inom samma kända bansträckningsfamilj');
...
assert.ok(classEvolutionSourceU7.includes('comparisonBreaks')&&!classEvolutionSourceU7.includes("class:'class-evolution-course-break'")&&classEvolutionSourceU7.includes('classTrailSegments(historical,this.model.years)')&&classEvolutionSourceU7.includes('Number(to.year)-Number(from.year)!==1'),'Klassutveckling ska behålla jämförbarhetslogik för analys, men rita beskrivande spår över efterföljande kalenderår och bryta vid verkliga luckor');
assert.ok(audienceSourceU7.includes('comparisonKeyForRace:historyComparisonKey')&&audienceSourceU7.includes('descriptiveHistoryRuns(valid,years)'),'Klassutveckling ska behålla U7-jämförbarhet medan Klasshistorik använder en beskrivande årsserie');
assert.ok(stylesSourceU5.includes('U7 Historik 2.0')&&stylesSourceU5.includes('.history-series'),'U7:s historikserier ska behålla sin visuella struktur även när tekniska brytlinjer döljs');
