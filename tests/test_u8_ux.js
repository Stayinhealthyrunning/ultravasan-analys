...
assert.ok(html.includes('id="genderHistoryChart"')&&html.includes('Deltagande och fullföljande över åren'),'U8 ska behålla könsuppdelad deltagande- och fullföljandehistorik som separat årsvy');
assert.ok(audience.includes('function renderGenderHistory()')&&audience.includes('stapel = startande · streckad linje = fullföljandegrad'),'årsvyn ska visa både startande och fullföljandegrad');
assert.ok(html.includes('När hade fältet gått i mål?')&&html.includes('id="percentileLadder"'),'primär finish progression ska ha egen tydlig vy');
for(const label of ['10 % i mål','25 % i mål','50 % i mål · median','75 % i mål','90 % i mål']){
  assert.ok(nerd.includes(label)&&audience.includes(label),'finish progression saknar beslutad nivå: '+label);
}
assert.ok(nerd.includes("all=rows.map(r=>Number(r.finish_seconds))")&&audience.includes("all=rows.map(r=>Number(r.finish_seconds))"),'finish progression ska beräkna huvudvärdet på aktiv FINISHED-kohort, inte endast separata könskohorter');
assert.ok(app.includes('Nivåerna motsvarar Q10, Q25, Q50, Q75 och Q90'),'metodhjälpen ska förklara finish progression matematiskt');
